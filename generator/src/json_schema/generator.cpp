...
namespace rpc_generator
{
    namespace json_schema
    {
                   ...
                   && current_entity_ptr->get_name() != "__global__")
            {
                ...
                if (owner == nullptr || owner->get_name().empty() || owner->get_name() == "__global__"
                    || (owner->get_entity_type() != entity_type::NAMESPACE
                        && owner->get_entity_type() != entity_type::CLASS && owner->get_entity_type() != entity_type::STRUCT
            ...
            const std::map<std::string, DefinitionInfoVariant>& definition_info_map)
        {
            ...
            std::string description = "Parameters for " + info.method_entity->get_name() + struct_type
                                      + " from interface " + info.interface_entity->get_name();
            ...
            const std::map<std::string, DefinitionInfoVariant>& definition_info_map)
        {
            ...
            if (parse_template_args(idl_type_name_cleaned, container_name, template_args))
            {
                ...
                else if (container_name == "std::array" && template_args.size() == 2)
                {
                    ...
                    {
                        std::cerr << "exception has occurred std::stoll(template_args[1]) has value "
                                  << template_args[1] << " resulting in this error: " << e.what() << "\n";
            ...
            if (is_int8(idl_type_name) || is_uint8(idl_type_name) || is_int16(idl_type_name) || is_uint16(idl_type_name)
                || is_int32(idl_type_name) || is_uint32(idl_type_name) || is_int64(idl_type_name)
                || is_uint64(idl_type_name) || is_long(idl_type_name) || is_ulong(idl_type_name)
                || idl_type_name == "int" || idl_type_name == "char")
            {
                writer.write_string_property("type", "integer");
            }
        ...
        void write_json_schema(const class_entity& root_entity, std::ostream& os, const std::string& schema_title)
        {
            ...
            while (!definitions_needed.empty() && processed_count++ < max_processed)
            {
                ...
                else
                {
                    ...
                    writer.write_string_property("type", "null");
                    writer.write_string_property(
                        "description", "Error: Definition info not found for '" + current_name + "'.");
                    writer.close_object();
                    definitions_written.insert(current_name);
                }
            }
