...
namespace rpc_generator
{
        ...
        {
            return is_int8(type) || is_uint8(type) || 
                   is_int16(type) || is_uint16(type) ||
                   is_int32(type) || is_uint32(type) ||
                   is_int64(type) || is_uint64(type) ||
                   is_long(type) || is_ulong(type) ||
                   type == "size_t" || type == "ptrdiff_t";
        }
                                       ...
                                       const attributes& attribs)
        {
            ...
            info.type = classify_parameter_type(type_name, info.reference_modifiers, 
                                              info.is_interface, info.is_out, 
                                              info.is_const, info.by_value);
        ...
        std::string base_renderer::render_param_type(param_type type, int option, bool from_host, const class_entity& lib, const std::string& name, 
                               ...
                               std::string& output)
        {
            ...
            try {
                output = renderer.render_param_type(info.type, option, from_host, lib, name, info.is_in, info.is_out, info.is_const, info.clean_type_name, count);
                return true;
                                ...
                                std::string& output)
        {
            ...
            try {
                switch (info.type) {
                    case param_type::BY_VALUE:
                        output = renderer.render_by_value(option, from_host, lib, name, info.is_in, info.is_out, info.is_const, info.clean_type_name, count);
                        break;
                    case param_type::REFERENCE:
                    ...
                    case param_type::POINTER_REFERENCE:
                        output = renderer.render_pointer_reference(option, from_host, lib, name, info.is_in, info.is_out, info.is_const, info.clean_type_name, count);
                        break;
                    case param_type::POINTER_POINTER:
                        output = renderer.render_pointer_pointer(option, from_host, lib, name, info.is_in, info.is_out, info.is_const, info.clean_type_name, count);
                        break;
                    case param_type::INTERFACE:
                        throw std::runtime_error("INTERFACE does not support out vals");
                    case param_type::INTERFACE_REFERENCE:
                        output = renderer.render_interface_reference(option, from_host, lib, name, info.is_in, info.is_out, info.is_const, info.clean_type_name, count);
                        break;
                    default:
                        std::cerr << fmt::format("Unsupported output parameter type for {} {} with modifiers {}", type, name, info.reference_modifiers);
                        throw fmt::format("Unsupported output parameter type for {} {} with modifiers {}", type, name, info.reference_modifiers);
                }
                return true;
            } catch (const std::exception& e) {
