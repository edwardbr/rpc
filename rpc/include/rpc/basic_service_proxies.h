...
namespace rpc
{
    ...
    class local_service_proxy : public service_proxy
    {
        ...
        local_service_proxy(
            const char* name, const std::shared_ptr<child_service>& child_svc, const std::shared_ptr<service>& parent_svc)
        ...
        std::shared_ptr<service_proxy> clone() override
        {
            return std::shared_ptr<service_proxy>(new local_service_proxy(*this));
        }
        ...
        static std::shared_ptr<local_service_proxy> create(const char* name,
            ...
            const std::shared_ptr<service>& parent_svc)
        {
    ...
    {
        std::shared_ptr<child_service> child_service_;
        ...
        local_child_service_proxy(
            const char* name, destination_zone destination_zone_id, const std::shared_ptr<service>& parent_svc, connect_fn fn)
        ...
        std::shared_ptr<service_proxy> clone() override
        {
            return std::shared_ptr<service_proxy>(new local_child_service_proxy(*this));
        }

        static std::shared_ptr<local_child_service_proxy> create(
            const char* name, destination_zone destination_zone_id, const std::shared_ptr<service>& svc, connect_fn fn)
        {
