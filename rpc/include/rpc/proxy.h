...
namespace rpc
{
    ...
    public:
        ...
        CORO_TASK(uint64_t) sp_release(object object_id)
        {
            ...
            while (version)
            {
                auto ret = CO_AWAIT release(version, destination_zone_id_, object_id, caller_zone_id_);
                if (ret != std::numeric_limits<uint64_t>::max())
                {
                    if (original_version != version)
                    {
        ...
        CORO_TASK(void) cleanup_after_object(object object_id, rpc::shared_ptr<service_proxy> keep_alive)
        {
            ...
            while (version)
            {
                auto ret = CO_AWAIT release(version, destination_zone_id_, object_id, caller_zone_id);
                if (ret != std::numeric_limits<uint64_t>::max())
                {
        ...
        rpc::shared_ptr<T>& iface)
    {
        ...
        else
        {
            ...
            if (is_new)
            {
                auto ret = CO_AWAIT service_proxy->sp_add_ref(encap.object_id, {0}, rpc::add_ref_options::normal);
                if (ret == std::numeric_limits<uint64_t>::max())
                    CO_RETURN - 1;
                if (!new_proxy_added)
                {
    ...
    template<class T>
    CORO_TASK(int)
    proxy_bind_out_param(const rpc::shared_ptr<rpc::service_proxy>& sp,
        const rpc::interface_descriptor& encap,
        caller_zone caller_zone_id,
