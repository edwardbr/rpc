...
namespace rpc
{
    ...
    public:
        ...
        interface_descriptor prepare_out_param(uint64_t protocol_version, caller_channel_zone caller_channel_zone_id, caller_zone caller_zone_id, const rpc::interface_proxy* base);
        ...
        interface_descriptor prepare_remote_input_interface(uint64_t protocol_version, caller_channel_zone caller_channel_zone_id, caller_zone caller_zone_id, const rpc::interface_proxy* base, std::shared_ptr<service_proxy>& destination_zone);
        
        void clean_up_on_failed_connection(const std::shared_ptr<service_proxy>& destination_zone, rpc::shared_ptr<rpc::casting_interface> input_interface);
        ...
        int connect_to_zone(
            ...
            Args&&... args)
        {
            RPC_ASSERT(input_interface == nullptr || !input_interface->get_interface_proxy() || input_interface->get_interface_proxy()->get_object_proxy()->get_service_proxy()->get_zone_id() == zone_id_);
            ...
            if(input_interface)
            {
                if(input_interface->get_interface_proxy())
                {
                    input_descr = prepare_remote_input_interface(rpc::get_version(), {0}, new_service_proxy->get_destination_zone_id().as_caller(), input_interface->get_interface_proxy(), destination_zone);   
                }
                else
                {
    ...
    public:
            ...
            , Args&&... args)
        {
            ...
            if(child_ptr)
            {
                RPC_ASSERT(!child_ptr->get_interface_proxy() && "we cannot support remote pointers to subordinate zones as it has not been registered yet");
