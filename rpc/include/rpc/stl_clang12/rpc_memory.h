...
// lifting and shifting from https://github.com/microsoft/STL/blob/main/stl/inc/memory
...
namespace ranges
{
    template<class _In, class _Out> using uninitialized_copy_result = in_out_result<_In, _Out>;
    ...
    public:
        ...
        uninitialized_copy_result<borrowed_iterator_t<_Rng1>, borrowed_iterator_t<_Rng2>> operator()(
            ...
            auto _UResult = _Uninitialized_copy_unchecked(_Get_unwrapped(_STD move(_First1)), _Uend(_Range1),
    ...
    private:
        ...
        _NODISCARD static uninitialized_copy_result<_It, _Out> _Uninitialized_copy_unchecked(_It _IFirst, _Se _ILast,
                                                                                             _Out _OFirst, _OSe _OLast)
        {
...
namespace ranges
{
    template<class _In, class _Out> using uninitialized_copy_n_result = in_out_result<_In, _Out>;
    ...
    public:
        ...
        uninitialized_copy_n_result<_It, _Out> operator()(
                         ...
                         && _Sized_or_unreachable_sentinel_for<_OSe, _Out>)
            {
                ...
                {
                    auto _UResult = _Copy_memcpy_common(_IFirst, _IFirst + _Count, _OFirst,
                                                        _RANGES next(_OFirst, _STD move(_OLast)));
    ...
    public:
        ...
        uninitialized_move_result<_It, _Out> operator()(_It _First1, _Se _Last1, _Out _First2, _OSe _Last2) const {
            ...
            auto _UResult = _RANGES _Uninitialized_move_unchecked(
                _Get_unwrapped(_STD move(_First1)), _Get_unwrapped(_STD move(_Last1)),
                _Get_unwrapped(_STD move(_First2)), _Get_unwrapped(_STD move(_Last2)));
        ...
        uninitialized_move_result<borrowed_iterator_t<_Rng1>, borrowed_iterator_t<_Rng2>> operator()(
            ...
            // clang-format on
            auto _First1 = _RANGES begin(_Range1);
            auto _UResult = _RANGES _Uninitialized_move_unchecked(_Get_unwrapped(_STD move(_First1)), _Uend(_Range1),
                                                                  _Ubegin(_Range2), _Uend(_Range2));
...
namespace ranges
{
    template<class _In, class _Out> using uninitialized_move_n_result = in_out_result<_In, _Out>;
    ...
    public:
        ...
        _It operator()(_It _First, _Se _Last, const _Ty& _Val) const {
            // clang-format on
            _Adl_verify_range(_First, _Last);
            auto _UResult = _Uninitialized_fill_unchecked(_Get_unwrapped(_STD move(_First)),
                                                          _Get_unwrapped(_STD move(_Last)), _Val);
...
namespace ranges
{
    ...
    public:
        ...
        template<destructible _Ty> constexpr void operator()(_Ty* const _Location) const noexcept
        {
...
#if _HAS_CXX17
template<class _NoThrowFwdIt> _CONSTEXPR20 void destroy(const _NoThrowFwdIt _First, const _NoThrowFwdIt _Last)
{
...
namespace ranges
{
    ...
    public:
        ...
        constexpr _It operator()(_It _First, _Se _Last) const noexcept {
            // clang-format on
            _Adl_verify_range(_First, _Last);
            _Seek_wrapped(_First, _RANGES _Destroy_unchecked(_Get_unwrapped(_STD move(_First)),
                                                             _Get_unwrapped(_STD move(_Last))));
            return _First;
        }
...
namespace ranges
{
    ...
    public:
        ...
        _It operator()(_It _First, _Se _Last) const {
            // clang-format on
            _Adl_verify_range(_First, _Last);
            auto _UResult = _Uninitialized_default_construct_unchecked(_Get_unwrapped(_STD move(_First)),
                                                                       _Get_unwrapped(_STD move(_Last)));
...
template<class _NoThrowFwdIt> void uninitialized_value_construct(const _NoThrowFwdIt _First, const _NoThrowFwdIt _Last)
{
...
namespace ranges
{
    ...
    public:
        ...
        _It operator()(_It _First, _Se _Last) const {
            // clang-format on
            _Adl_verify_range(_First, _Last);
            auto _UResult = _Uninitialized_value_construct_unchecked(_Get_unwrapped(_STD move(_First)),
                                                                     _Get_unwrapped(_STD move(_Last)));
...
template<class _OutIt, class _Ty> class _CXX17_DEPRECATE_RAW_STORAGE_ITERATOR raw_storage_iterator
...
public:
    ...
    _NODISCARD _OutIt base() const { return _Next; }
...
public:
    ...
    ~auto_ptr() noexcept { delete _Myptr; }
    ...
    _NODISCARD _Ty* get() const noexcept { return _Myptr; }
...
private:
    ...
    virtual void _Destroy() noexcept { _STD terminate(); }

    virtual void _Delete_this() noexcept { _STD terminate(); }
...
public:
    ...
    __attribute__((no_sanitize("thread"))) bool _Incref_nz() noexcept
    ...
    long _Use_count() const noexcept { return static_cast<long>(_Uses); }

    virtual void* _Get_deleter(const ::std::type_info&) const noexcept { return nullptr; }
};
...
public:
    _Ref_count_resource(_Resource _Px, _Dx _Dt)
        : _Ref_count_base()
        , _Mypair(_One_then_variadic_args_t {}, _STD move(_Dt), _Px)
    {
    }
    ...
    void* _Get_deleter(const ::std::type_info& _Typeid) const noexcept override
    {
#if _HAS_STATIC_RTTI
...
struct _Can_enable_shared<_Yty, ::std::void_t<typename _Yty::_Esft_type>>
    : ::std::is_convertible<remove_cv_t<_Yty>*, typename _Yty::_Esft_type*>::type
{
    // ::std::is_convertible is necessary to verify unambiguous inheritance
};
...
public:
    ...
    template<class _Ty2> _NODISCARD bool owner_before(const _Ptr_base<_Ty2>& _Right) const noexcept
...
protected:
    _NODISCARD element_type* get() const noexcept { return _Ptr; }
    ...
    template<class _Ty0> friend class weak_ptr; // specifically, weak_ptr::lock()
    ...
    template<class _Ty2> void _Weakly_convert_rvalue_avoiding_expired_conversions(_Ptr_base<_Ty2>&& _Other) noexcept
    {
...
private:
    ...
    template<class _Ty0> friend struct atomic;
...
template<class _Yty> struct _Can_scalar_delete<_Yty, ::std::void_t<decltype(delete _STD declval<_Yty*>())>> : true_type
{
};
...
template<class _Yty> struct _Can_array_delete<_Yty, ::std::void_t<decltype(delete[] _STD declval<_Yty*>())>> : true_type
{
};
...
template<class _Yty, class _Ty> struct _SP_convertible : ::std::is_convertible<_Yty*, _Ty*>::type
{
};
...
template<class _Yty, class _Ty> struct _SP_pointer_compatible : ::std::is_convertible<_Yty*, _Ty*>::type
{
...
template<class _Ux> struct _Temporary_owner
{
    ...
    explicit _Temporary_owner(_Ux* const _Ptr_) noexcept
        : _Ptr(_Ptr_)
    {
    }
...
template<class _UxptrOrNullptr, class _Dx> struct _Temporary_owner_del
{
    ...
    explicit _Temporary_owner_del(const _UxptrOrNullptr _Ptr_, _Dx& _Dt_) noexcept
        : _Ptr(_Ptr_)
        , _Dt(_Dt_)
    {
    }
...
public:
    ...
    template<class _Ux, enable_if_t<::std::conjunction_v<conditional_t<::std::is_array_v<_Ty>, _Can_array_delete<_Ux>,
    ...
    template<class _Dx, enable_if_t<::std::conjunction_v<::std::is_move_constructible<_Dx>,
                                                         _Can_call_function_object<_Dx&, ::std::nullptr_t&>>,
                                    int>
                        = 0>
    ...
    template<class _Dx, class _Alloc,
             enable_if_t<::std::conjunction_v<::std::is_move_constructible<_Dx>,
                                              _Can_call_function_object<_Dx&, ::std::nullptr_t&>>,
                         int>
             = 0>
...
#if _HAS_AUTO_PTR_ETC
    template<class _Ty2, enable_if_t<::std::is_convertible_v<_Ty2*, _Ty*>, int> = 0> shared_ptr(auto_ptr<_Ty2>&& _Other)
    ...
    shared_ptr(unique_ptr<_Ux, _Dx>&& _Other)
    {
        ...
        using _Deleter_t = conditional_t<::std::is_reference_v<_Dx>, decltype(_STD ref(_Other.get_deleter())), _Dx>;
...
private:
    ...
    friend enable_if_t<!::std::is_array_v<_Ty0>, shared_ptr<_Ty0>> allocate_shared(const _Alloc& _Al_arg,
                                                                                   _Types&&... _Args);

    template<class _Ty0> friend enable_if_t<is_bounded_array_v<_Ty0>, shared_ptr<_Ty0>> make_shared();
    ...
    make_shared(const ::std::remove_extent_t<_Ty0>& _Val);

    template<class _Ty0, class _Alloc>
    friend enable_if_t<is_bounded_array_v<_Ty0>, shared_ptr<_Ty0>>
    allocate_shared(const _Alloc& _Al_arg, const ::std::remove_extent_t<_Ty0>& _Val);

    template<class _Ty0> friend enable_if_t<!is_unbounded_array_v<_Ty0>, shared_ptr<_Ty0>> make_shared_for_overwrite();
    ...
    friend shared_ptr<_Ty0> _Allocate_shared_unbounded_array(const _Alloc& _Al, size_t _Count,
                                                             const _ArgTypes&... _Args);
    ...
    { // take ownership of _Px
        this->_Ptr = _Px;
        this->_Rep = _Rx;
        if constexpr(::std::conjunction_v<::std::negation<::std::is_array<_Ty>>,
                                          ::std::negation<::std::is_volatile<_Ux>>, _Can_enable_shared<_Ux>>)
        {
    ...
    void _Set_ptr_rep_and_enable_shared(::std::nullptr_t, _Ref_count_base* const _Rx) noexcept
...
#if _HAS_CXX17
template<class _Ty> shared_ptr(weak_ptr<_Ty>) -> shared_ptr<_Ty>;

template<class _Ty, class _Dx> shared_ptr(unique_ptr<_Ty, _Dx>) -> shared_ptr<_Ty>;
#endif // _HAS_CXX17
...
template<class _Ty> _NODISCARD bool operator==(const shared_ptr<_Ty>& _Left, ::std::nullptr_t) noexcept
{
    return _Left.get() == nullptr;
}

#if _HAS_CXX20
template<class _Ty> _NODISCARD strong_ordering operator<=>(const shared_ptr<_Ty>& _Left, ::std::nullptr_t) noexcept
{
    return _Left.get() <=> static_cast<typename shared_ptr<_Ty>::element_type*>(nullptr);
}
#else  // ^^^ _HAS_CXX20 / !_HAS_CXX20 vvv
template<class _Ty> _NODISCARD bool operator==(::std::nullptr_t, const shared_ptr<_Ty>& _Right) noexcept
{
...
template<class _Ty1, class _Ty2> _NODISCARD shared_ptr<_Ty1> static_pointer_cast(shared_ptr<_Ty2>&& _Other) noexcept
{
    // only work with local polymorphic types
    //  static_cast for shared_ptr that properly respects the reference count control block
    const auto _Ptr = static_cast<typename shared_ptr<_Ty1>::element_type*>(_Other.get());
...
template<class _Ty1, class _Ty2> _NODISCARD shared_ptr<_Ty1> const_pointer_cast(const shared_ptr<_Ty2>& _Other) noexcept
{
...
template<class _Ty1, class _Ty2> _NODISCARD shared_ptr<_Ty1> const_pointer_cast(shared_ptr<_Ty2>&& _Other) noexcept
{
    // const_cast for shared_ptr that properly respects the reference count control block
    const auto _Ptr = const_cast<typename shared_ptr<_Ty1>::element_type*>(_Other.get());
    return shared_ptr<_Ty1>(_STD move(_Other), _Ptr);
...
template<class _Ty1, class _Ty2> _NODISCARD shared_ptr<_Ty1> dynamic_pointer_cast(shared_ptr<_Ty2>&& _Other) noexcept
{
#ifdef _CPPRTTI
    // dynamic_cast for shared_ptr that properly respects the reference count control block
    const auto _Ptr = dynamic_cast<typename shared_ptr<_Ty1>::element_type*>(_Other.get());
...
#if _HAS_STATIC_RTTI
template<class _Dx, class _Ty> _NODISCARD _Dx* get_deleter(const shared_ptr<_Ty>& _Sx) noexcept
{
...
#else  // _HAS_STATIC_RTTI
template<class _Dx, class _Ty> _Dx* get_deleter(const shared_ptr<_Ty>&) noexcept = delete; // requires static RTTI
#endif // _HAS_STATIC_RTTI
...
public:
    ...
    explicit _Ref_count_obj2(_Types&&... _Args)
        : _Ref_count_base()
    {
#if _HAS_CXX20
...
#if _HAS_CXX20
template<size_t _Align> struct _Alignas_storage_unit
{
    alignas(_Align) char _Space[_Align];
};

enum class _Check_overflow : bool
{
    _Nope,
    _Yes
};
...
template<class _Refc> _NODISCARD _Refc* _Allocate_flexible_array(const size_t _Count)
{
...
template<class _Refc> void _Deallocate_flexible_array(_Refc* const _Ptr) noexcept
{
    constexpr size_t _Align = alignof(_Refc);
#ifdef __cpp_aligned_new
    if constexpr(_Align > __STDCPP_DEFAULT_NEW_ALIGNMENT__)
...
template<class _NoThrowIt> struct _NODISCARD _Uninitialized_rev_destroying_backout
{
...
template<class _Ty> void _Reverse_destroy_multidimensional_n(_Ty* const _Arr, size_t _Size) noexcept
{
    while(_Size > 0)
    {
        --_Size;
...
template<class _Ty> struct _NODISCARD _Reverse_destroy_multidimensional_n_guard
{
    _Ty* _Target;
    size_t _Index;
...
template<class _Ty, size_t _Size> void _Uninitialized_copy_multidimensional(const _Ty (&_In)[_Size], _Ty (&_Out)[_Size])
{
...
template<class _Ty> void _Uninitialized_value_construct_multidimensional_n(_Ty* const _Out, const size_t _Size)
{
...
template<class _Ty> void _Uninitialized_default_construct_multidimensional_n(_Ty* const _Out, const size_t _Size)
{
    if constexpr(!::std::is_trivially_default_constructible_v<_Ty>)
    {
        if constexpr(::std::is_array_v<_Ty>)
...
template<class _Ty> void _Uninitialized_fill_multidimensional_n(_Ty* const _Out, const size_t _Size, const _Ty& _Val)
{
    if constexpr(::std::is_array_v<_Ty>)
    {
        _Reverse_destroy_multidimensional_n_guard<_Ty> _Guard {_Out, 0};
...
public:
    ...
    explicit _Ref_count_unbounded_array(const size_t _Count)
        : _Ref_count_base()
    {
    ...
    template<class _Arg>
    explicit _Ref_count_unbounded_array(const size_t _Count, const _Arg& _Val)
        : _Ref_count_base()
    {
    ...
    _NODISCARD auto _Get_ptr() noexcept { return _STD addressof(_Storage._Value); }
...
template<class _Ty> class _Ref_count_unbounded_array<_Ty, false> : public _Ref_count_base
{
...
template<class _Ty> class _Ref_count_bounded_array : public _Ref_count_base
{
...
public:
    ...
    _Ref_count_bounded_array()
        ...
        , _Storage()
    {
    } // value-initializing _Storage is necessary here

    template<class _Arg>
    explicit _Ref_count_bounded_array(const _Arg& _Val)
        : _Ref_count_base()
    { // don't value-initialize _Storage
        if constexpr(::std::is_same_v<_For_overwrite_tag, _Arg>)
        {
...
template<class _Ty, bool = ::std::is_empty_v<_Ty> && !::std::is_final_v<_Ty>> class _Ebco_base : private _Ty
...
protected:
    ...
    constexpr _Ty& _Get_val() noexcept { return *this; }

    constexpr const _Ty& _Get_val() const noexcept { return *this; }
};
...
protected:
    ...
    constexpr _Ty& _Get_val() noexcept { return _Myval; }

    constexpr const _Ty& _Get_val() const noexcept { return _Myval; }
};
...
#endif // _HAS_CXX20
        {
            allocator_traits<_Rebound>::construct(this->_Get_val(), _STD addressof(_Storage._Value),
...
#if _HAS_CXX20
template<class _Alloc> class _NODISCARD _Uninitialized_rev_destroying_backout_al
{
...
template<class _Ty, class _Alloc> struct _NODISCARD _Reverse_destroy_multidimensional_n_al_guard
{
    _Ty* _Target;
    size_t _Index;
    _Alloc& _Al;
...
void _Uninitialized_fill_multidimensional_n_al(_Ty* const _Out, const size_t _Size, const _Ty& _Val, _Alloc& _Al)
{
    ...
    {
        if constexpr(_Fill_zero_memset_is_safe<_Ty*, _Ty> && _Uses_default_construct<_Alloc, _Ty*, const _Ty&>::value)
        {
            if(_Is_all_bits_zero(_Val))
            {
                _Fill_zero_memset(_Out, _Size);
...
private:
    ...
    void _Destroy() noexcept override
    { // destroy managed resource
        if constexpr(!::std::conjunction_v<::std::is_trivially_destructible<_Item>,
                                           _Uses_default_destroy<_Rebound, _Item*>>)
        {
...
public:
    ...
    { // don't value-initialize _Storage
        if constexpr(::std::is_same_v<_For_overwrite_tag, _Arg>)
        {
            _Uninitialized_default_construct_multidimensional_n(_Storage._Value,
                                                                ::std::extent_v<_Ty>); // the allocator isn't needed
        }
        else
        {
...
#if _HAS_CXX20
template<class _Refc> struct _NODISCARD _Global_delete_guard
{
    _Refc* _Target;
...
template<class _Ty> _NODISCARD enable_if_t<is_unbounded_array_v<_Ty>, shared_ptr<_Ty>> make_shared(const size_t _Count)
{
...
_NODISCARD enable_if_t<is_unbounded_array_v<_Ty>, shared_ptr<_Ty>> make_shared(const size_t _Count,
                                                                               const ::std::remove_extent_t<_Ty>& _Val)
{
    return _Make_shared_unbounded_array<_Ty>(_Count, _Val);
}

template<class _Ty> _NODISCARD enable_if_t<is_bounded_array_v<_Ty>, shared_ptr<_Ty>> make_shared()
{
...
template<class _Ty> _NODISCARD enable_if_t<!is_unbounded_array_v<_Ty>, shared_ptr<_Ty>> make_shared_for_overwrite()
{
    shared_ptr<_Ty> _Ret;
    if constexpr(::std::is_array_v<_Ty>)
    {
...
#if _HAS_CXX20
template<class _Alloc> struct _Allocate_n_ptr
{
    _Alloc& _Al;
...
_NODISCARD shared_ptr<_Ty> _Allocate_shared_unbounded_array(const _Alloc& _Al, const size_t _Count,
                                                            const _ArgTypes&... _Args)
{
    // make a shared_ptr to an unbounded array
    static_assert(is_unbounded_array_v<_Ty>);
...
template<class _Ty, class _Alloc>
_NODISCARD enable_if_t<is_unbounded_array_v<_Ty>, shared_ptr<_Ty>> allocate_shared(const _Alloc& _Al,
                                                                                   const size_t _Count)
{
...
template<class _Ty, class _Alloc>
_NODISCARD enable_if_t<is_unbounded_array_v<_Ty>, shared_ptr<_Ty>>
allocate_shared(const _Alloc& _Al, const size_t _Count, const ::std::remove_extent_t<_Ty>& _Val)
{
...
template<class _Ty, class _Alloc>
_NODISCARD enable_if_t<is_bounded_array_v<_Ty>, shared_ptr<_Ty>>
allocate_shared(const _Alloc& _Al, const ::std::remove_extent_t<_Ty>& _Val)
{
    // make a shared_ptr to a bounded array
    using _Refc = _Ref_count_bounded_array_alloc<remove_cv_t<_Ty>, _Alloc>;
    using _Alblock = _Rebind_alloc_t<_Alloc, _Refc>;
...
public:
    ...
    template<class _Ty2, class = const _Ty2*> static constexpr bool _Must_avoid_expired_conversions_from = true;
    ...
    weak_ptr(weak_ptr&& _Other) noexcept { this->_Move_construct_from(_STD move(_Other)); }
    ...
    void swap(weak_ptr& _Other) noexcept { this->_Swap(_Other); }
...
#if _HAS_CXX17
template<class _Ty> weak_ptr(shared_ptr<_Ty>) -> weak_ptr<_Ty>;
#endif // _HAS_CXX17
...
public:
    ...
    _NODISCARD shared_ptr<_Ty> shared_from_this() { return shared_ptr<_Ty>(_Wptr); }

    _NODISCARD shared_ptr<const _Ty> shared_from_this() const { return shared_ptr<const _Ty>(_Wptr); }

    _NODISCARD weak_ptr<_Ty> weak_from_this() noexcept { return _Wptr; }

    _NODISCARD weak_ptr<const _Ty> weak_from_this() const noexcept { return _Wptr; }
...
using _Unique_ptr_enable_default_t
    = enable_if_t<::std::conjunction_v<::std::negation<::std::is_pointer<_Dx2>>, ::std::is_default_constructible<_Dx2>>,
                  int>;
...
public:
    ...
    template<
        class _Dx2 = _Dx,
        enable_if_t<
            ::std::conjunction_v<::std::negation<::std::is_reference<_Dx2>>, ::std::is_constructible<_Dx2, _Dx2>>, int>
        = 0>
    ...
    template<class _Dx2 = _Dx,
             enable_if_t<::std::conjunction_v<::std::is_reference<_Dx2>,
                                              ::std::is_constructible<_Dx2, remove_reference_t<_Dx2>>>,
                         int>
             = 0>
    ...
    template<class _Ty2, class _Dx2,
             enable_if_t<::std::conjunction_v<::std::negation<::std::is_array<_Ty2>>,
                                              ::std::is_convertible<typename unique_ptr<_Ty2, _Dx2>::pointer, pointer>,
                                              conditional_t<::std::is_reference_v<_Dx>, is_same<_Dx2, _Dx>,
    ...
    template<
        class _Ty2,
        enable_if_t<::std::conjunction_v<::std::is_convertible<_Ty2*, _Ty*>, is_same<_Dx, default_delete<_Ty>>>, int>
        = 0>
    unique_ptr(auto_ptr<_Ty2>&& _Right) noexcept
        : _Mypair(_Zero_then_variadic_args_t {}, _Right.release())
    {
    }
#endif // _HAS_AUTO_PTR_ETC
    ...
    _NODISCARD _Dx& get_deleter() noexcept { return _Mypair._Get_first(); }
    ...
    _NODISCARD pointer operator->() const noexcept { return _Mypair._Myval2; }

    _NODISCARD pointer get() const noexcept { return _Mypair._Myval2; }
...
public:
    ...
    using _Enable_conversion = enable_if_t<::std::conjunction_v<
        ::std::is_array<_Uty>, is_same<pointer, element_type*>, is_same<_UP_pointer, _UP_element_type*>,
        ::std::is_convertible<_UP_element_type (*)[], element_type (*)[]>, _More>>; // TRANSITION, GH-248

    template<
        class _Uty, class _Ex,
        class = _Enable_conversion<
            _Uty, _Ex, conditional_t<::std::is_reference_v<_Dx>, is_same<_Ex, _Dx>, ::std::is_convertible<_Ex, _Dx>>>>
...
_NODISCARD unique_ptr<_Ty> make_unique(const size_t _Size)
{ // make a unique_ptr
...
_NODISCARD unique_ptr<_Ty> make_unique_for_overwrite(const size_t _Size)
{ // make a unique_ptr with default initialization
    using _Elem = ::std::remove_extent_t<_Ty>;
    return unique_ptr<_Ty>(new _Elem[_Size]);
}
...
template<class _Ty, class _Dx> _NODISCARD bool operator==(const unique_ptr<_Ty, _Dx>& _Left, ::std::nullptr_t) noexcept
{
    return !_Left;
}

#if !_HAS_CXX20
template<class _Ty, class _Dx> _NODISCARD bool operator==(::std::nullptr_t, const unique_ptr<_Ty, _Dx>& _Right) noexcept
{
    return !_Right;
}

template<class _Ty, class _Dx>
_NODISCARD bool operator!=(const unique_ptr<_Ty, _Dx>& _Left, ::std::nullptr_t _Right) noexcept
{
    return !(_Left == _Right);
}

template<class _Ty, class _Dx>
_NODISCARD bool operator!=(::std::nullptr_t _Left, const unique_ptr<_Ty, _Dx>& _Right) noexcept
{
    return !(_Left == _Right);
}
#endif // !_HAS_CXX20

template<class _Ty, class _Dx> _NODISCARD bool operator<(const unique_ptr<_Ty, _Dx>& _Left, ::std::nullptr_t _Right)
{
    using _Ptr = typename unique_ptr<_Ty, _Dx>::pointer;
    return ::std::less<_Ptr> {}(_Left.get(), _Right);
}

template<class _Ty, class _Dx> _NODISCARD bool operator<(::std::nullptr_t _Left, const unique_ptr<_Ty, _Dx>& _Right)
{
    using _Ptr = typename unique_ptr<_Ty, _Dx>::pointer;
...
struct _Can_stream_unique_ptr<_OutTy, _PxTy,
                              ::std::void_t<decltype(_STD declval<_OutTy>() << _STD declval<_PxTy>().get())>>
    : true_type
{
};

// template <class _Elem, class _Traits, class _Yty, class _Dx,
//     enable_if_t<_Can_stream_unique_ptr<basic_ostream<_Elem, _Traits>&, const unique_ptr<_Yty, _Dx>&>::value, int> =
//     0>
...
template<class _Ty = void> struct owner_less; // not defined
...
template<class _Ty, class _Dx> struct ::std::hash<rpc::unique_ptr<_Ty, _Dx>>
{
...
template<class _Ty> struct ::std::hash<rpc::shared_ptr<_Ty>>
{
...
#if _HAS_CXX20
template<size_t _Nx, class _Ty> _NODISCARD constexpr _Ty* assume_aligned(_Ty* const _Ptr) noexcept /* strengthened */
{
...
template<class _Ty>
_CXX20_DEPRECATE_OLD_SHARED_PTR_ATOMIC_SUPPORT _NODISCARD shared_ptr<_Ty>
atomic_load_explicit(const shared_ptr<_Ty>* _Ptr, memory_order)
{
...
template<class _Ty>
_CXX20_DEPRECATE_OLD_SHARED_PTR_ATOMIC_SUPPORT _NODISCARD shared_ptr<_Ty> atomic_load(const shared_ptr<_Ty>* _Ptr)
{ // load *_Ptr atomically
    return _STD atomic_load_explicit(_Ptr, memory_order_seq_cst);
}

template<class _Ty>
_CXX20_DEPRECATE_OLD_SHARED_PTR_ATOMIC_SUPPORT void atomic_store_explicit(shared_ptr<_Ty>* _Ptr, shared_ptr<_Ty> _Other,
                                                                          memory_order)
{
...
template<class _Ty>
_CXX20_DEPRECATE_OLD_SHARED_PTR_ATOMIC_SUPPORT void atomic_store(shared_ptr<_Ty>* _Ptr, shared_ptr<_Ty> _Other)
{ // store _Other to *_Ptr atomically
    _STD atomic_store_explicit(_Ptr, _STD move(_Other), memory_order_seq_cst);
}

template<class _Ty>
_CXX20_DEPRECATE_OLD_SHARED_PTR_ATOMIC_SUPPORT shared_ptr<_Ty>
atomic_exchange_explicit(shared_ptr<_Ty>* _Ptr, shared_ptr<_Ty> _Other, memory_order)
{
...
template<class _Ty>
_CXX20_DEPRECATE_OLD_SHARED_PTR_ATOMIC_SUPPORT shared_ptr<_Ty> atomic_exchange(shared_ptr<_Ty>* _Ptr,
...
template<class _Ty>
_CXX20_DEPRECATE_OLD_SHARED_PTR_ATOMIC_SUPPORT bool
atomic_compare_exchange_weak_explicit(shared_ptr<_Ty>* _Ptr, shared_ptr<_Ty>* _Exp, shared_ptr<_Ty> _Value,
...
template<class _Ty>
_CXX20_DEPRECATE_OLD_SHARED_PTR_ATOMIC_SUPPORT bool
atomic_compare_exchange_weak(shared_ptr<_Ty>* _Ptr, shared_ptr<_Ty>* _Exp, shared_ptr<_Ty> _Value)
{
    // atomically compare and exchange
    return _STD atomic_compare_exchange_weak_explicit(_Ptr, _Exp, _STD move(_Value), memory_order_seq_cst,
                                                      memory_order_seq_cst);
}

template<class _Ty>
_CXX20_DEPRECATE_OLD_SHARED_PTR_ATOMIC_SUPPORT bool
atomic_compare_exchange_strong_explicit(shared_ptr<_Ty>* _Ptr, shared_ptr<_Ty>* _Exp, shared_ptr<_Ty> _Value,
                                        memory_order, memory_order)
{ // atomically compare and exchange
    return _STD atomic_compare_exchange_weak_explicit(_Ptr, _Exp, _STD move(_Value), memory_order_seq_cst,
                                                      memory_order_seq_cst);
...
template<class _Ty>
_CXX20_DEPRECATE_OLD_SHARED_PTR_ATOMIC_SUPPORT bool
atomic_compare_exchange_strong(shared_ptr<_Ty>* _Ptr, shared_ptr<_Ty>* _Exp, shared_ptr<_Ty> _Value)
{
    // atomically compare and exchange
    return _STD atomic_compare_exchange_strong_explicit(_Ptr, _Exp, _STD move(_Value), memory_order_seq_cst,
                                                        memory_order_seq_cst);
}

#if _HAS_CXX20
template<class _Ty> class alignas(2 * sizeof(void*)) _Atomic_ptr_base
{
...
protected:
    ...
    void notify_one() noexcept { _Ptr.notify_one(); }

    void notify_all() noexcept { _Ptr.notify_all(); }

    atomic<_Ty*> _Ptr {nullptr};
    mutable _Locked_pointer<_Ref_count_base> _Repptr;
};

template<class _Ty> struct atomic<shared_ptr<_Ty>> : private _Atomic_ptr_base<_Ty>
{
private:
...
public:
    ...
    _NODISCARD bool is_lock_free() const noexcept { return false; }
    ...
    operator shared_ptr<_Ty>() const noexcept { return load(); }
    ...
    atomic(const shared_ptr<_Ty> _Value) noexcept
        : _Base(_Value._Ptr, _Value._Rep)
    {
        _Value._Incref();
    }
    ...
    void operator=(shared_ptr<_Ty> _Value) noexcept { store(_STD move(_Value)); }
...
template<class _Ty> struct atomic<weak_ptr<_Ty>> : private _Atomic_ptr_base<_Ty>
{
private:
    using _Base = _Atomic_ptr_base<_Ty>;
...
public:
    ...
    operator weak_ptr<_Ty>() const noexcept { return load(); }
    ...
    bool compare_exchange_weak(weak_ptr<_Ty>& _Expected, weak_ptr<_Ty> _Desired,
                               const memory_order _Order = memory_order_seq_cst) noexcept
    {
        return compare_exchange_strong(_Expected, _STD move(_Desired), _Order);
    }

    bool compare_exchange_strong(weak_ptr<_Ty>& _Expected, weak_ptr<_Ty> _Desired,
                                 const memory_order _Order = memory_order_seq_cst) noexcept
    {
        _Check_memory_order(_Order);
        auto _Rep = this->_Repptr._Lock_and_load();
    ...
    atomic(const weak_ptr<_Ty> _Value) noexcept
        : _Base(_Value._Ptr, _Value._Rep)
    {
        _Value._Incwref();
    }
    ...
    void operator=(weak_ptr<_Ty> _Value) noexcept { store(_STD move(_Value)); }
...
template<class _Ty> struct _Pointer_of_helper
{
};

template<_Has_member_pointer _Ty> struct _Pointer_of_helper<_Ty>
{
...
template<class _Ty> using _Pointer_of = typename _Pointer_of_helper<_Ty>::type;

template<class _Ty, class _Uty> struct _Pointer_of_or_helper
{
    using type = _Uty;
};
...
template<class _Ty, class _Uty> using _Pointer_of_or = typename _Pointer_of_or_helper<_Ty, _Uty>::type;
...
template<class _SmartPtr, class _Pointer, class... _ArgsT> class out_ptr_t
{
...
public:
    ...
    operator _Pointer*() const noexcept { return _STD addressof(_Get_ptr()); }

    operator void**() const noexcept
        requires(!::std::is_same_v<_Pointer, void*>)
    {
        static_assert(::std::is_pointer_v<_Pointer>,
                      "conversion of out_ptr_t<Smart, Pointer, Args...> to void** requires "
...
private:
    ...
    _NODISCARD tuple<_ArgsT...>& _Get_args() noexcept { return _Mypair._Get_first(); }
...
template<class _SmartPtr, class _Pointer, class... _ArgsT> class inout_ptr_t
{
...
private:
    _NODISCARD static auto _Get_ptr_from_smart(_SmartPtr& _Smart_ptr) noexcept
        requires ::std::is_pointer_v<_SmartPtr>
    {
        return _Smart_ptr;
    }
...
public:
    explicit inout_ptr_t(_SmartPtr& _Smart_ptr_,
                         _ArgsT... _Args_) noexcept(::std::is_nothrow_constructible_v<tuple<_ArgsT...>, _ArgsT...>
    ...
    {
        static_assert(::std::is_pointer_v<_Pointer>,
                      "conversion of inout_ptr_t<Smart, Pointer, Args...> to void** requires "
