...
namespace ranges
{
    ...
    public:
        ...
        uninitialized_copy_result<_It, _Out> operator()(_It _First1, _Se _Last1, _Out _First2, _OSe _Last2) const {
            ...
            auto _UResult = _Uninitialized_copy_unchecked(_Get_unwrapped(_STD move(_First1)),
                _Get_unwrapped(_STD move(_Last1)),
                _Get_unwrapped(_STD move(_First2)),
    ...
    private:
            ...
            _It _IFirst, _Se _ILast, _Out _OFirst, _OSe _OLast)
        {
                          ...
                          && _Sized_or_unreachable_sentinel_for<_OSe, _Out>)
            {
                ...
                {
                    return _Copy_memcpy_common(_IFirst,
                        _RANGES next(_IFirst, _STD move(_ILast)),
                        _OFirst,
...
namespace ranges
{
    ...
    inline constexpr _Uninitialized_value_construct_fn uninitialized_value_construct{_Not_quite_object::_Construct_tag{}};
} // namespace ranges
#endif // __cpp_lib_concepts
...
struct _Can_call_function_object<_Fx, _Arg, ::std::void_t<decltype(_STD declval<_Fx>()(_STD declval<_Arg>()))>> : true_type
{
};
...
public:
    ...
    template<class _Ux,
        class _Dx,
        enable_if_t<::std::conjunction_v<::std::is_move_constructible<_Dx>, _Can_call_function_object<_Dx&, _Ux*&>, _SP_convertible<_Ux, _Ty>>, int>
        = 0>
...
private:
    ...
    friend enable_if_t<!::std::is_array_v<_Ty0>, shared_ptr<_Ty0>> allocate_shared(
    ...
    template<class _Ty0, class _Alloc>
    friend enable_if_t<!is_unbounded_array_v<_Ty0>, shared_ptr<_Ty0>> allocate_shared_for_overwrite(const _Alloc& _Al_arg);
    ...
    friend shared_ptr<_Ty0> _Allocate_shared_unbounded_array(const _Alloc& _Al, size_t _Count, const _ArgTypes&... _Args);
    ...
    { // take ownership of _Px
        ...
        if constexpr (::std::conjunction_v<::std::negation<::std::is_array<_Ty>>,
                          ::std::negation<::std::is_volatile<_Ux>>,
...
_NODISCARD shared_ptr<_Ty1> dynamic_pointer_cast(const shared_ptr<_Ty2>& _Other) noexcept
{
    ...
    auto ob = casting_interface::get_object_proxy(*proxy_);
    shared_ptr<_Ty1> ret;
...
_NODISCARD shared_ptr<_Ty> _Allocate_shared_unbounded_array(const _Alloc& _Al, const size_t _Count, const _ArgTypes&... _Args)
{
    // make a shared_ptr to an unbounded array
    static_assert(is_unbounded_array_v<_Ty>);
...
template<class _Ty, class _Alloc>
_NODISCARD enable_if_t<is_unbounded_array_v<_Ty>, shared_ptr<_Ty>> allocate_shared(const _Alloc& _Al, const size_t _Count)
{
...
public:
    ...
    template<class _Uty, class _Dx2 = _Dx, enable_if_t<::std::is_constructible_v<_Dx2, const _Dx2&>, int> = 0, class = _Enable_ctor_reset<_Uty>>
    ...
    using _Enable_conversion = enable_if_t<::std::conjunction_v<::std::is_array<_Uty>,
        is_same<pointer, element_type*>,
        is_same<_UP_pointer, _UP_element_type*>,
        ::std::is_convertible<_UP_element_type (*)[], element_type (*)[]>,
        _More>>; // TRANSITION, GH-248

    template<class _Uty,
        class _Ex,
        class = _Enable_conversion<_Uty, _Ex, conditional_t<::std::is_reference_v<_Dx>, is_same<_Ex, _Dx>, ::std::is_convertible<_Ex, _Dx>>>>
...
template<class _Ty>
_CXX20_DEPRECATE_OLD_SHARED_PTR_ATOMIC_SUPPORT shared_ptr<_Ty> atomic_exchange(shared_ptr<_Ty>* _Ptr, shared_ptr<_Ty> _Other)
{
    // copy _Other to *_Ptr and return previous value of *_Ptr atomically
...
template<class _Ty>
_CXX20_DEPRECATE_OLD_SHARED_PTR_ATOMIC_SUPPORT bool atomic_compare_exchange_weak_explicit(
    shared_ptr<_Ty>* _Ptr, shared_ptr<_Ty>* _Exp, shared_ptr<_Ty> _Value, memory_order, memory_order)
...
template<class _Ty>
_CXX20_DEPRECATE_OLD_SHARED_PTR_ATOMIC_SUPPORT bool atomic_compare_exchange_strong_explicit(
    shared_ptr<_Ty>* _Ptr, shared_ptr<_Ty>* _Exp, shared_ptr<_Ty> _Value, memory_order, memory_order)
{ // atomically compare and exchange
    return _STD atomic_compare_exchange_weak_explicit(
        _Ptr, _Exp, _STD move(_Value), memory_order_seq_cst, memory_order_seq_cst);
...
public:
    ...
    bool compare_exchange_weak(
        shared_ptr<_Ty>& _Expected, shared_ptr<_Ty> _Desired, const memory_order _Success, const memory_order _Failure) noexcept
    {
    ...
    bool compare_exchange_weak(
        shared_ptr<_Ty>& _Expected, shared_ptr<_Ty> _Desired, const memory_order _Order = memory_order_seq_cst) noexcept
    {
...
public:
    ...
    bool compare_exchange_weak(
        weak_ptr<_Ty>& _Expected, weak_ptr<_Ty> _Desired, const memory_order _Success, const memory_order _Failure) noexcept
    {
        return compare_exchange_strong(_Expected, _STD move(_Desired), _Combine_cas_memory_orders(_Success, _Failure));
    }
...
public:
    explicit inout_ptr_t(_SmartPtr& _Smart_ptr_, _ArgsT... _Args_) noexcept(
        ::std::is_nothrow_constructible_v<tuple<_ArgsT...>, _ArgsT...>
        && noexcept(_Get_ptr_from_smart(_Smart_ptr_))) /* strengthened */
        : _Smart_ptr(_Smart_ptr_)
        , _Mypair(_One_then_variadic_args_t{},
              tuple<_ArgsT...>{_STD forward<_ArgsT>(_Args_)...},
