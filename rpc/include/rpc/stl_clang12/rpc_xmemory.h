...
#pragma once
#ifndef _RPC_XMEMORY_
#define _RPC_XMEMORY_
...
//     _RPC_VERIFY(_Ptr_container != 0, "invalid argument"); // validate even in release since we're doing p[-1]
...
//     _RPC_VERIFY(_Back_shift >= _Min_back_shift && _Back_shift <= _Non_user_size, "invalid argument");
...
// _CONSTEXPR20 _Pointer _Refancy(typename std::pointer_traits<_Pointer>::element_type* _Ptr) noexcept {
...
template <class _Ty>
_CONSTEXPR20 void _rpc_Destroy_in_place(_Ty& _Obj) noexcept {
...
//     using type   = typename std::pointer_traits<_Ptrty>::template rebind<const _Valty>;
// };
...
//     using _Ptrty = typename _Get_pointer_type<_Ty>::type;
//     using type   = typename std::pointer_traits<_Ptrty>::template rebind<void>;
// };
...
//     using _Ptrty = typename _Get_pointer_type<_Ty>::type;
//     using type   = typename std::pointer_traits<_Ptrty>::template rebind<const void>;
// };
...
//     using _Ptrty = typename _Get_pointer_type<_Ty>::type;
//     using type   = typename std::pointer_traits<_Ptrty>::difference_type;
// };
...
//         _RPC_ASSERT(_Ptr != nullptr || _Count == 0, "null pointer cannot point to a block of non-zero size");
//         // no overflow check on the following multiply; we assume _Allocate did that check
...
//     } else {
//         _RPC_ASSERT(_Left == _Right, "containers incompatible for swap");
//     }
// }
...
//     } else {
//         using _Ptr_traits = std::pointer_traits<_Alloc_ptr_t<_Alloc>>;
...
//         _rpc_Construct_in_place(*_New_proxy, this);
//         _Myproxy            = _New_proxy;
...
//         _RPC_VERIFY(*_Pnext, "ITERATOR LIST CORRUPTED!");
//         *_Pnext  = _Mynextiter;
//         _Myproxy = nullptr;
//     }
...
//         _Ptr = _Unfancy(_Al_.allocate(1));
//         _rpc_Construct_in_place(*_Ptr, _STD addressof(_Mycont));
...
template <class _Ty1, class _Ty2, bool = ::std::is_empty<_Ty1>::value 
    //&& !std::is_final<_Ty1>::value
    >
...
//         _rpc_Construct_in_place(*_Last, _STD forward<_Types>(_Vals)...);
//         ++_Last;
//     }
...
//             } else {
//                 _RPC_ASSERT(false, "Tried to uninitialized_move two ranges with unreachable sentinels");
//             }
//         } else {
...
_NODISCARD constexpr bool _Allocators_equal(const _Alloc& _Lhs, const _Alloc& _Rhs) noexcept {
    if constexpr (std::allocator_traits<_Alloc>::is_always_equal::value) {
        return true;
    } else {
        return _Lhs == _Rhs;
    }
...
//     } else if constexpr (::std::is_constructible_v<_Ty, std::allocator_arg_t, const _Alloc&, _Types...>) {
//         using _ReturnType = tuple<std::allocator_arg_t, const _Alloc&, _Types&&...>;
//         return _ReturnType{std::allocator_arg, _Al, _STD forward<_Types>(_Args)...};
...
//             "T must be constructible from either (std::allocator_arg_t, const Alloc&, Types...) "
...
// // template <class _Pointer, enable_if_t<!::std::is_pointer_v<_Pointer>, int> = 0>
// // _CONSTEXPR20 _Pointer _Refancy(typename std::pointer_traits<_Pointer>::element_type* _Ptr) noexcept {
// //     return std::pointer_traits<_Pointer>::pointer_to(*_Ptr);
// // }
...
// template <class _Ptrty>
// auto _Const_cast(_Ptrty _Ptr) noexcept { // remove constness from a fancy pointer
//     using _Elem       = typename std::pointer_traits<_Ptrty>::element_type;
//     using _Modifiable = remove_const_t<_Elem>;
//     using _Dest       = typename std::pointer_traits<_Ptrty>::template rebind<_Modifiable>;

//     return std::pointer_traits<_Dest>::pointer_to(const_cast<_Modifiable&>(*_Ptr));
// }
