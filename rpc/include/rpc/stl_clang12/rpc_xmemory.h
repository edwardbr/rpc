...
template<class _Ty> struct _NODISCARD _Tidy_guard
...
template<class _Ty> struct _NODISCARD _Tidy_deallocate_guard
...
template<size_t _Ty_size> _NODISCARD constexpr size_t _Get_size_of_n(const size_t _Count)
{
...
template<class _Ty>
_INLINE_VAR constexpr size_t _New_alignof
    = (_STD max)(alignof(_Ty),
...
template<class _Traits> void* _Allocate_manually_vector_aligned(const size_t _Bytes)
{
...
template<class _Ty> _CONSTEXPR20 void _Destroy_in_place(_Ty& _Obj) noexcept
{
...
#if _HAS_CXX17
template<class _Ty> _CONSTEXPR20 void destroy_at(_Ty* const _Location) noexcept /* strengthened */
{
#if _HAS_CXX20
    if constexpr(::std::is_array_v<_Ty>)
    {
...
template<class _Ty> struct _Get_pointer_type<_Ty, ::std::void_t<typename _Ty::pointer>>
{
...
_STL_DISABLE_DEPRECATED_WARNING
template<class _Ty> struct _Get_const_pointer_type<_Ty, ::std::void_t<typename _Ty::const_pointer>>
{
...
template<class _Ty> struct _Get_void_pointer_type<_Ty, ::std::void_t<typename _Ty::void_pointer>>
{
...
template<class _Ty> struct _Get_const_void_pointer_type<_Ty, ::std::void_t<typename _Ty::const_void_pointer>>
{
...
template<class _Ty> struct _Get_difference_type<_Ty, ::std::void_t<typename _Ty::difference_type>>
{
...
template<class _Ty> struct _Get_size_type<_Ty, ::std::void_t<typename _Ty::size_type>>
{
...
_STL_DISABLE_DEPRECATED_WARNING
template<class _Ty> struct _Get_is_always_equal<_Ty, ::std::void_t<typename _Ty::is_always_equal>>
{
...
struct _Has_no_allocator_construct<
    ::std::void_t<decltype(_STD declval<_Alloc&>().construct(_STD declval<_Ptr>(), _STD declval<_Args>()...))>, _Alloc,
    _Ptr, _Args...> : false_type
{
};
_STL_RESTORE_DEPRECATED_WARNING
...
struct _Has_no_alloc_destroy<_Alloc, _Ptr,
                             ::std::void_t<decltype(_STD declval<_Alloc&>().destroy(_STD declval<_Ptr>()))>>
    : false_type
{
};
_STL_RESTORE_DEPRECATED_WARNING
...
template<class _Alloc, class _Size_type, class _Const_void_pointer, class = void> struct _Has_allocate_hint : false_type
{
};
...
template<class _Alloc, class = void> struct _Has_max_size : false_type
{
};
...
template<class _Alloc, class = void> struct _Has_select_on_container_copy_construction : false_type
{
};

template<class _Alloc>
struct _Has_select_on_container_copy_construction<
    _Alloc, ::std::void_t<decltype(_STD declval<const _Alloc&>().select_on_container_copy_construction())>> : true_type
{
};
...
{ // defines traits for allocators
    ...
    _NODISCARD static _CONSTEXPR20 pointer allocate(_Alloc& _Al, _CRT_GUARDOVERFLOW const size_type _Count)
    {
    ...
    static _CONSTEXPR20 void deallocate(_Alloc& _Al, pointer _Ptr, size_type _Count) { _Al.deallocate(_Ptr, _Count); }

    template<class _Ty, class... _Types> static _CONSTEXPR20 void construct(_Alloc& _Al, _Ty* _Ptr, _Types&&... _Args)
    {
    ...
    template<class _Ty> static _CONSTEXPR20 void destroy(_Alloc& _Al, _Ty* _Ptr)
    {
...
{ // traits for std::allocator
    ...
    _NODISCARD static _CONSTEXPR20 _Alloc select_on_container_copy_construction(const _Alloc& _Al) { return _Al; }
};
...
using _Maybe_rebind_alloc_t =
    typename _Select<::std::is_same_v<typename _Alloc::value_type, _Value_type>>::template _Apply<
        _Alloc&, _Rebind_alloc_t<_Alloc, _Value_type>>;

template<class _Alloc> // tests if allocator has simple addressing
_INLINE_VAR constexpr bool _Is_simple_alloc_v
    = ::std::is_same_v<typename allocator_traits<_Alloc>::size_type, size_t>
...
#if _HAS_CXX23
template<class _Ptr> struct allocation_result
{
    _Ptr ptr;
    size_t count;
};
...
template<class _Alloc>
_NODISCARD constexpr allocation_result<typename allocator_traits<_Alloc>::pointer>
allocate_at_least(_Alloc& _Al, const size_t _Count)
{
...
public:
...
#if _HAS_DEPRECATED_ALLOCATOR_MEMBERS
    _CXX17_DEPRECATE_OLD_ALLOCATOR_MEMBERS _NODISCARD _Ty* allocate(_CRT_GUARDOVERFLOW const size_t _Count, const void*)
    {
    ...
    template<class _Uty> _CXX17_DEPRECATE_OLD_ALLOCATOR_MEMBERS void destroy(_Uty* const _Ptr) { _Ptr->~_Uty(); }
...
#if !_HAS_CXX20
template<class _Ty, class _Other> _NODISCARD bool operator!=(const allocator<_Ty>&, const allocator<_Other>&) noexcept
{
    return false;
}
#endif // !_HAS_CXX20
...
template<class _Alloc> _CONSTEXPR20 void _Pocca(_Alloc& _Left, const _Alloc& _Right) noexcept
{
...
template<class _Alloc> _CONSTEXPR20 void _Pocma(_Alloc& _Left, _Alloc& _Right) noexcept
...
template<class _Alloc> _CONSTEXPR20 void _Pocs(_Alloc& _Left, _Alloc& _Right) noexcept
{
...
{ // pointer used to help construct 1 _Alloc::value_type without EH
    using pointer = _Alloc_ptr_t<_Alloc>;
    _Alloc& _Al;
    pointer _Ptr;

    _CONSTEXPR20 explicit _Alloc_construct_ptr(_Alloc& _Al_)
        : _Al(_Al_)
        , _Ptr(nullptr)
    {
    }

    _NODISCARD _CONSTEXPR20 pointer _Release() noexcept
    { // disengage *this and return contained pointer
        return _STD exchange(_Ptr, nullptr);
    }

    _CONSTEXPR20 void _Allocate()
    {                   // disengage *this, then allocate a new memory block
        _Ptr = nullptr; // if allocate throws, prevents double-free
        _Ptr = _Al.allocate(1);
    }

    _CONSTEXPR20 ~_Alloc_construct_ptr()
...
struct _Iterator_base0
{
    ...
    _CONSTEXPR20 const _Container_base0* _Getcont() const noexcept { return nullptr; }
...
{ // store head of iterator chain and back pointer
    ...
    _CONSTEXPR20 _Container_proxy(_Container_base12* _Mycont_) noexcept
        : _Mycont(_Mycont_)
    {
    }
...
public:
    ...
    template<class _Alloc> _CONSTEXPR20 void _Alloc_proxy(_Alloc&& _Al)
    {
    ...
    template<class _Alloc> _CONSTEXPR20 void _Reload_proxy(_Alloc&& _Old_alloc, _Alloc&& _New_alloc)
    {
...
public:
    ...
    _CONSTEXPR20 _Iterator_base12(const _Iterator_base12& _Right) noexcept { *this = _Right; }
    ...
    _CONSTEXPR20 const _Container_base12* _Getcont() const noexcept { return _Myproxy ? _Myproxy->_Mycont : nullptr; }
...
{ // fake replacement for a container proxy smart pointer when no container proxy is in use
...
struct _Basic_container_proxy_ptr12
{
    ...
    { // disengage this _Basic_container_proxy_ptr12
        _Ptr = nullptr;
    }
...
template<class _Alloc> struct _Container_proxy_ptr12 : _Basic_container_proxy_ptr12
{
    ...
    _CONSTEXPR20 _Container_proxy_ptr12(_Alloc& _Al_, _Leave_proxy_unbound)
        : _Al(_Al_)
    {
    ...
    _CONSTEXPR20 _Container_proxy_ptr12(_Alloc& _Al_, _Container_base12& _Mycont)
        : _Al(_Al_)
    {
...
{ // store a ::std::pair of values, deriving from empty first
public:
    _Ty2 _Myval2;
...
{ // store a ::std::pair of values, not deriving from first
public:
    _Ty1 _Myval1;
    _Ty2 _Myval2;
...
template<class _Ty>::std::pair<_Ty*, ptrdiff_t> _Get_temporary_buffer(ptrdiff_t _Count) noexcept
{
...
template<class _NoThrowFwdIt> struct _NODISCARD _Uninitialized_backout
{
    ...
    _CONSTEXPR20 ~_Uninitialized_backout() { _Destroy_range(_First, _Last); }

    template<class... _Types> _CONSTEXPR20 void _Emplace_back(_Types&&... _Vals)
    {
...
namespace ranges
{
    ...
    in_out_result<_InIt, _OutIt> _Copy_memcpy_distance(_InIt _IFirst, _OutIt _OFirst, const _DistIt _DFirst,
                                                       const _DistIt _DLast) noexcept
    {
    ...
    template<class _InIt, class _OutIt>
    in_out_result<_InIt, _OutIt> _Copy_memcpy_common(_InIt _IFirst, _InIt _ILast, _OutIt _OFirst,
                                                     _OutIt _OLast) noexcept
    {
        const auto _IFirstPtr = _To_address(_IFirst);
    ...
    template<class _In, class _Out> using uninitialized_move_result = in_out_result<_In, _Out>;
...
template<class _Alloc> class _NODISCARD _Uninitialized_backout_al
{
    // struct to undo partially constructed ranges in _Uninitialized_xxx_al algorithms
    using pointer = _Alloc_ptr_t<_Alloc>;

public:
    _CONSTEXPR20 _Uninitialized_backout_al(pointer _Dest, _Alloc& _Al_)
        : _First(_Dest)
        , _Last(_Dest)
        , _Al(_Al_)
    {
    ...
    _CONSTEXPR20 ~_Uninitialized_backout_al() { _Destroy_range(_First, _Last, _Al); }
...
_CONSTEXPR20 _Alloc_ptr_t<_Alloc> _Uninitialized_copy(const _InIt _First, const _InIt _Last, _Alloc_ptr_t<_Alloc> _Dest,
                                                      _Alloc& _Al)
{
...
template<class _InIt, class _Alloc>
_CONSTEXPR20 _Alloc_ptr_t<_Alloc> _Uninitialized_move(const _InIt _First, const _InIt _Last, _Alloc_ptr_t<_Alloc> _Dest,
                                                      _Alloc& _Al)
{
...
template<class _Alloc>
_CONSTEXPR20 _Alloc_ptr_t<_Alloc> _Uninitialized_fill_n(_Alloc_ptr_t<_Alloc> _First, _Alloc_size_t<_Alloc> _Count,
...
_INLINE_VAR constexpr bool _Use_memset_value_construct_v
    = ::std::conjunction_v<bool_constant<_Iterator_is_contiguous<_NoThrowFwdIt>>,
                           ::std::is_scalar<_Iter_value_t<_NoThrowFwdIt>>,
...
template<class _Alloc>
_CONSTEXPR20 _Alloc_ptr_t<_Alloc> _Uninitialized_value_construct_n(_Alloc_ptr_t<_Alloc> _First,
                                                                   _Alloc_size_t<_Alloc> _Count, _Alloc& _Al)
{
...
template<class _Ty>
_CXX17_DEPRECATE_TEMPORARY_BUFFER _NODISCARD ::std::pair<_Ty*, ptrdiff_t>
get_temporary_buffer(ptrdiff_t _Count) noexcept
{
    return _Get_temporary_buffer<_Ty>(_Count);
}

template<class _Ty> _CXX17_DEPRECATE_TEMPORARY_BUFFER void return_temporary_buffer(_Ty* _Pbuf)
{
...
template<class _Key> struct _In_place_key_extract_set<_Key, _Key>
{
    ...
    static const _Key& _Extract(const _Key& _Val) noexcept { return _Val; }
};

// assumes _Args have already been _Remove_cvref_t'd
template<class _Key, class... _Args> struct _In_place_key_extract_map
{
    // by default we can't extract the key in the emplace family and must construct a node we might not use
    static constexpr bool _Extractable = false;
};
...
template<class _Key, class _Second> struct _In_place_key_extract_map<_Key, _Key, _Second>
{
    ...
    static constexpr bool _Extractable = true;
    static const _Key& _Extract(const _Key& _Val, const _Second&) noexcept { return _Val; }
};

template<class _Key, class _First, class _Second> struct _In_place_key_extract_map<_Key, ::std::pair<_First, _Second>>
{
    // if we would call the ::std::pair(::std::pair<other, other>) constructor family, we can use the ::std::pair.first
    // member as the key
    static constexpr bool _Extractable = ::std::is_same_v<_Key, _Remove_cvref_t<_First>>;
    static const _Key& _Extract(const ::std::pair<_First, _Second>& _Val) { return _Val.first; }
};
...
template<class _Alloc> struct _Alloc_temporary2
{
    using value_type = typename _Alloc::value_type;
    ...
    _NODISCARD _CONSTEXPR20 value_type& _Get_value() noexcept { return _Storage._Value; }

    _NODISCARD _CONSTEXPR20 const value_type& _Get_value() const noexcept { return _Storage._Value; }
    ...
    _NODISCARD _CONSTEXPR20 value_type& _Get_value() noexcept { return _Value; }

    _NODISCARD _CONSTEXPR20 const value_type& _Get_value() const noexcept { return _Value; }
#endif // ^^^ no workaround ^^^
    ...
    _CONSTEXPR20 ~_Alloc_temporary2() { _Traits::destroy(_Al, _STD addressof(_Get_value())); }
};

template<class _Alloc> _NODISCARD constexpr bool _Allocators_equal(const _Alloc& _Lhs, const _Alloc& _Rhs) noexcept
{
...
template<class _FwdIt, class _Pr> _NODISCARD _CONSTEXPR20 _FwdIt remove_if(_FwdIt _First, const _FwdIt _Last, _Pr _Pred)
{
...
template<class _Container, class _Pr> typename _Container::size_type _Erase_nodes_if(_Container& _Cont, _Pr _Pred)
{
    // erase each element satisfying _Pred
    auto _First = _Cont.begin();
    const auto _Last = _Cont.end();
...
template<class _Ty, class _Alloc, class _Uty1, class _Uty2,
         enable_if_t<_Is_specialization_v<_Ty, ::std::pair>, int> = 0>
_NODISCARD constexpr auto uses_allocator_construction_args(const _Alloc& _Al, _Uty1&& _Val1, _Uty2&& _Val2) noexcept;

template<class _Ty, class _Alloc, class _Uty1, class _Uty2,
         enable_if_t<_Is_specialization_v<_Ty, ::std::pair>, int> = 0>
_NODISCARD constexpr auto uses_allocator_construction_args(const _Alloc& _Al,
                                                           const ::std::pair<_Uty1, _Uty2>& _Pair) noexcept;

template<class _Ty, class _Alloc, class _Uty1, class _Uty2,
         enable_if_t<_Is_specialization_v<_Ty, ::std::pair>, int> = 0>
_NODISCARD constexpr auto uses_allocator_construction_args(const _Alloc& _Al,
                                                           ::std::pair<_Uty1, _Uty2>&& _Pair) noexcept;

template<class _Ty, class _Alloc, class _Tuple1, class _Tuple2,
         enable_if_t<_Is_specialization_v<_Ty, ::std::pair>, int> = 0>
_NODISCARD constexpr auto uses_allocator_construction_args(const _Alloc& _Al, piecewise_construct_t, _Tuple1&& _Tup1,
                                                           _Tuple2&& _Tup2) noexcept
{
...
{
    return _STD apply([&](auto&&... _Construct_args)
