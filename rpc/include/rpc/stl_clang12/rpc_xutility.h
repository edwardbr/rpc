...
_DECLSPEC_NOALIAS void __cdecl __std_swap_ranges_trivially_swappable_noalias(void* _First1, void* _Last1,
...
template<class _Ty>
inline constexpr bool _Is_trivially_swappable_v
    = std::conjunction_v<::std::is_trivially_destructible<_Ty>, std::is_trivially_move_constructible<_Ty>,
                         std::is_trivially_move_assignable<_Ty>, std::negation<_Has_ADL_swap<_Ty>>>;

#ifdef __cpp_lib_byte
template<> inline constexpr bool _Is_trivially_swappable_v<::std::byte> = true;
...
template<
    class _To, class _From,
    ::std::enable_if_t<::std::conjunction_v<::std::bool_constant<sizeof(_To) == sizeof(_From)>,
                                            ::std::is_trivially_copyable<_To>, ::std::is_trivially_copyable<_From>>,
                       int>
    = 0>
...
template<class _Ty> struct _Get_element_type<_Ty, ::std::void_t<typename _Ty::element_type>>
{
...
template<class _Ty> struct _Get_ptr_difference_type<_Ty, ::std::void_t<typename _Ty::difference_type>>
{
...
template<class _Iter> _NODISCARD constexpr void* _Voidify_iter(_Iter _It) noexcept
{
...
#if _HAS_CXX20
template<class _Ty, class... _Types, class = ::std::void_t<decltype(::new(declval<void*>()) _Ty(declval<_Types>()...))>>
...
_CONSTEXPR20 void _Construct_in_place(_Ty& _Obj,
...
template<class _Ty> void _Default_construct_in_place(_Ty& _Obj) noexcept(::std::is_nothrow_default_constructible_v<_Ty>)
{
...
template<class _Ty> struct pointer_traits<_Ty*>
{
    ...
    _NODISCARD static _CONSTEXPR20 pointer pointer_to(_Reftype _Val) noexcept { return _STD addressof(_Val); }
};
...
inline constexpr bool
    _Has_to_address_v<_Ty, ::std::void_t<decltype(pointer_traits<_Ty>::to_address(declval<const _Ty&>()))>>
    = true;

template<class _Ty> _NODISCARD constexpr _Ty* to_address(_Ty* const _Val) noexcept
{
...
template<class _Ptr> _NODISCARD constexpr auto to_address(const _Ptr& _Val) noexcept
{
...
struct identity
{
    template<class _Ty> _NODISCARD constexpr _Ty&& operator()(_Ty&& _Left) const noexcept
    {
...
_INLINE_VAR constexpr bool _Pass_functor_by_value_v
    = ::std::conjunction_v<::std::bool_constant<sizeof(_Fn) <= sizeof(void*)>,
...
template<class _Ty> using _Algorithm_int_t = ::std::conditional_t<::std::is_integral_v<_Ty>, _Ty, ptrdiff_t>;
...
using iter_difference_t = typename ::std::conditional_t<_Is_from_primary<iterator_traits<remove_cvref_t<_Ty>>>,
                                                        incrementable_traits<remove_cvref_t<_Ty>>,
...
template<_Has_member_value_type _Ty> struct indirectly_readable_traits<_Ty> : _Cond_value_type<typename _Ty::value_type>
{
};
...
using iter_value_t = typename ::std::conditional_t<_Is_from_primary<iterator_traits<remove_cvref_t<_Ty>>>,
                                                   indirectly_readable_traits<remove_cvref_t<_Ty>>,
                                                   iterator_traits<remove_cvref_t<_Ty>>>::value_type;

template<_Dereferenceable _Ty> using iter_reference_t = decltype(*declval<_Ty&>());
...
{
    template<class _It> using _Apply = typename _It::pointer;
};
...
{
    template<class _It> using _Apply = typename incrementable_traits<_It>::difference_type;
};
...
enum class _Itraits_pointer_strategy
{
    _Use_void,
    _Use_member,
    _Use_decltype
...
{
    template<class _It> using _Apply = decltype(declval<_It&>().operator->());
};
...
{
    template<class _It> using _Apply = typename _It::reference;
};
...
{
    template<class _It> using _Apply = typename _Iter_traits_category4<_Cpp17_random_delta<_It>>::type;
};
...
{
    template<class _It> using _Apply = typename _Iter_traits_category3<_Cpp17_bidi_delta<_It>>::template _Apply<_It>;
};
...
{
    template<class _It> using _Apply = typename _It::iterator_category;
};

template<> struct _Iter_traits_category<false>
{
    template<class _It> using _Apply = typename _Iter_traits_category2<_Cpp17_forward_delta<_It>>::template _Apply<_It>;
};
...
template<class _Ty> struct iterator_traits : _Iterator_traits_base<_Ty>
{
...
template<class _Ty> using _Make_unsigned_like_t = ::std::make_unsigned_t<_Ty>;

template<_Integer_like _Ty> _NODISCARD constexpr auto _To_unsigned_like(const _Ty _Value) noexcept
{
    return static_cast<_Make_unsigned_like_t<_Ty>>(_Value);
}

template<class _Ty> using _Make_signed_like_t = make_signed_t<_Ty>;

template<_Integer_like _Ty> _NODISCARD constexpr auto _To_signed_like(const _Ty _Value) noexcept
{
...
template<class _Se, class _It> inline constexpr bool disable_sized_sentinel_for = false;
...
template<bool _Iterator_category_present> struct _Iter_concept_impl2
{
    template<class _It, class _Traits> using _Apply = typename _Traits::iterator_category;
};
template<> struct _Iter_concept_impl2<false>
{
    // clang-format off
...
template<bool _Iterator_concept_present> struct _Iter_concept_impl1
{
    template<class _It, class _Traits> using _Apply = typename _Traits::iterator_concept;
};
template<> struct _Iter_concept_impl1<false>
{
    template<class _It, class _Traits>
...
template<class _It,
         class _Traits = ::std::conditional_t<_Is_from_primary<iterator_traits<_It>>, _It, iterator_traits<_It>>>
using _Iter_concept =
...
template<indirectly_readable _It, indirectly_regular_unary_invocable<_It> _Proj> struct projected
{
...
template<weakly_incrementable _It, class _Proj> struct incrementable_traits<projected<_It, _Proj>>
{
...
namespace ranges
{
    ...
    {
        template<class _Ty1, class _Ty2> void iter_swap(_Ty1, _Ty2) = delete;
        ...
        private:
            ...
            template<class _Ty1, class _Ty2> _NODISCARD static _CONSTEVAL _Choice_t<_St> _Choose() noexcept
            {
                ...
                else if constexpr(_Symmetric_indirectly_movable_storable<_Ty1, _Ty2>)
                {
                    constexpr auto _Nothrow
                        = noexcept(*declval<_Ty1>() = _Iter_exchange_move(declval<_Ty2>(), declval<_Ty1>()));
            ...
            template<class _Ty1, class _Ty2> static constexpr _Choice_t<_St> _Choice = _Choose<_Ty1, _Ty2>();
...
template<class _Iter> using _Iter_ref_t = iter_reference_t<_Iter>;

template<class _Iter> using _Iter_value_t = iter_value_t<_Iter>;

template<class _Iter> using _Iter_diff_t = iter_difference_t<_Iter>;
...
struct _Iterator_traits_base<
    _Iter, ::std::void_t<typename _Iter::iterator_category, typename _Iter::value_type, typename _Iter::difference_type,
...
template<class _Ty, bool = ::std::is_object_v<_Ty>> struct _Iterator_traits_pointer_base
...
template<class... _Iters> using _Common_diff_t = ::std::common_type_t<_Iter_diff_t<_Iters>...>;

template<class _Iter> using _Iter_cat_t = typename iterator_traits<_Iter>::iterator_category;

template<class _Ty, class = void> _INLINE_VAR constexpr bool _Is_iterator_v = false;

template<class _Ty> _INLINE_VAR constexpr bool _Is_iterator_v<_Ty, ::std::void_t<_Iter_cat_t<_Ty>>> = true;

template<class _Ty> struct _Is_iterator : ::std::bool_constant<_Is_iterator_v<_Ty>>
{
};
...
template<class _Ty> constexpr void _Verify_range(const _Ty* const _First, const _Ty* const _Last) noexcept
{
...
template<class _Iter, class = void> _INLINE_VAR constexpr bool _Allow_inheriting_unwrap_v = true;
...
template<class _Iter, class _Sentinel = _Iter, class = void> _INLINE_VAR constexpr bool _Range_verifiable_v = false;

template<class _Iter, class _Sentinel>
_INLINE_VAR constexpr bool _Range_verifiable_v<
    _Iter, _Sentinel, ::std::void_t<decltype(_Verify_range(declval<const _Iter&>(), declval<const _Sentinel&>()))>>
...
template<class _Iter, class = void> _INLINE_VAR constexpr bool _Unwrappable_v = false;

template<class _Iter>
_INLINE_VAR constexpr bool _Unwrappable_v<
    _Iter, ::std::void_t<decltype(declval<_Remove_cvref_t<_Iter>&>()._Seek_to(declval<_Iter>()._Unwrapped()))>>
    = _Allow_inheriting_unwrap_v<_Remove_cvref_t<_Iter>>;

template<class _Iter> _NODISCARD constexpr decltype(auto) _Get_unwrapped(_Iter&& _It)
{
...
template<class _Iter> using _Unwrapped_t = _Remove_cvref_t<decltype(_Get_unwrapped(declval<_Iter>()))>;

template<class _Iter, class = bool> _INLINE_VAR constexpr bool _Do_unwrap_when_unverified_v = false;
...
template<class _Iter> _NODISCARD constexpr decltype(auto) _Get_unwrapped_unverified(_Iter&& _It)
{
...
template<class _Diff> _INLINE_VAR constexpr _Diff _Max_possible_v {static_cast<::std::make_unsigned_t<_Diff>>(-1) >> 1};

template<class _Diff> _INLINE_VAR constexpr _Diff _Min_possible_v {-_Max_possible_v<_Diff> - 1};

template<class _Iter, class = void> _INLINE_VAR constexpr bool _Offset_verifiable_v = false;
...
template<class _Iter, class _Diff> _NODISCARD constexpr decltype(auto) _Get_unwrapped_n(_Iter&& _It, const _Diff _Off)
{
    if constexpr(::std::is_pointer_v<::std::decay_t<_Iter>>)
    {
        return _It + 0;
...
template<class _Iter, class _UIter, class = void> _INLINE_VAR constexpr bool _Wrapped_seekable_v = false;
...
#if _HAS_CXX17
template<class _Ty, class = void> struct _Is_allocator : false_type
{
}; // selected when _Ty can't possibly be an allocator

template<class _Ty>
struct _Is_allocator<_Ty,
                     ::std::void_t<typename _Ty::value_type, decltype(declval<_Ty&>().deallocate(
...
template<class _Iter> using _Guide_key_t = remove_const_t<typename iterator_traits<_Iter>::value_type::first_type>;

template<class _Iter> using _Guide_val_t = typename iterator_traits<_Iter>::value_type::second_type;
...
template<class _Ty> inline constexpr bool is_execution_policy_v = is_execution_policy<_Ty>::value;
...
template<class _Checked, class _Iter> _NODISCARD constexpr auto _Idl_distance(const _Iter& _First, const _Iter& _Last)
{
...
template<class _Elem, bool _Is_enum = ::std::is_enum_v<_Elem>> struct _Unwrap_enum
...
constexpr bool _Debug_lt_pred(_Pr&& _Pred, _Ty1&& _Left,
                              _Ty2&& _Right) noexcept(noexcept(_Pred(_Left, _Right)) && noexcept(_Pred(_Right, _Left)))
{
...
// from <iterator>
template<class _InIt, class _Diff> _CONSTEXPR17 void advance(_InIt& _Where, _Diff _Off)
...
template<class _InIt> _NODISCARD _CONSTEXPR17 _Iter_diff_t<_InIt> distance(_InIt _First, _InIt _Last)
{
    if constexpr(_Is_random_iter_v<_InIt>)
    {
...
template<class _InIt> _NODISCARD _CONSTEXPR17 _InIt next(_InIt _First, _Iter_diff_t<_InIt> _Off = 1)
{ // increment iterator
...
template<class _BidIt> _NODISCARD _CONSTEXPR17 _BidIt prev(_BidIt _First, _Iter_diff_t<_BidIt> _Off = 1)
{ // decrement iterator
...
public:
    ...
    friend constexpr void iter_swap(const reverse_iterator& _Left, const reverse_iterator<_BidIt2>& _Right) noexcept(
        is_nothrow_copy_constructible_v<_BidIt> && is_nothrow_copy_constructible_v<_BidIt2>
        && noexcept(_RANGES iter_swap(--declval<_BidIt&>(), --declval<_BidIt2&>())))
    {
    ...
    _NODISCARD constexpr const _BidIt& _Get_current() const noexcept { return current; }
...
_NODISCARD constexpr compare_three_way_result_t<_BidIt1, _BidIt2> operator<=>(const reverse_iterator<_BidIt1>& _Left,
...
_NODISCARD _CONSTEXPR17 auto operator-(const reverse_iterator<_BidIt1>& _Left,
                                       const reverse_iterator<_BidIt2>& _Right) -> decltype(_Right._Get_current()
                                                                                            - _Left._Get_current())
{
    return _Right._Get_current() - _Left._Get_current();
}
...
template<class _BidIt>
_NODISCARD _CONSTEXPR17 reverse_iterator<_BidIt>
make_reverse_iterator(_BidIt _Iter) noexcept(::std::is_nothrow_move_constructible_v<_BidIt>) /* strengthened */
{
...
template<class _Container> _NODISCARD _CONSTEXPR17 auto begin(_Container& _Cont) -> decltype(_Cont.begin())
{
    return _Cont.begin();
}

template<class _Container> _NODISCARD _CONSTEXPR17 auto begin(const _Container& _Cont) -> decltype(_Cont.begin())
{
    return _Cont.begin();
}

template<class _Container> _NODISCARD _CONSTEXPR17 auto end(_Container& _Cont) -> decltype(_Cont.end())
{
    return _Cont.end();
}

template<class _Container> _NODISCARD _CONSTEXPR17 auto end(const _Container& _Cont) -> decltype(_Cont.end())
{
    return _Cont.end();
}

template<class _Ty, size_t _Size> _NODISCARD constexpr _Ty* begin(_Ty (&_Array)[_Size]) noexcept
{
    return _Array;
}

template<class _Ty, size_t _Size> _NODISCARD constexpr _Ty* end(_Ty (&_Array)[_Size]) noexcept
{
    return _Array + _Size;
}
...
cbegin(const _Container& _Cont) noexcept(noexcept(_RPC begin(_Cont))) -> decltype(_RPC begin(_Cont))
{
    return _RPC begin(_Cont);
}

template<class _Container>
_NODISCARD constexpr auto cend(const _Container& _Cont) noexcept(noexcept(_RPC end(_Cont))) -> decltype(_RPC end(_Cont))
{
    return _RPC end(_Cont);
}

template<class _Container> _NODISCARD _CONSTEXPR17 auto rbegin(_Container& _Cont) -> decltype(_Cont.rbegin())
{
    return _Cont.rbegin();
}

template<class _Container> _NODISCARD _CONSTEXPR17 auto rbegin(const _Container& _Cont) -> decltype(_Cont.rbegin())
{
    return _Cont.rbegin();
}

template<class _Container> _NODISCARD _CONSTEXPR17 auto rend(_Container& _Cont) -> decltype(_Cont.rend())
{
    return _Cont.rend();
}

template<class _Container> _NODISCARD _CONSTEXPR17 auto rend(const _Container& _Cont) -> decltype(_Cont.rend())
{
    return _Cont.rend();
}

template<class _Ty, size_t _Size> _NODISCARD _CONSTEXPR17 reverse_iterator<_Ty*> rbegin(_Ty (&_Array)[_Size])
{
    return reverse_iterator<_Ty*>(_Array + _Size);
}

template<class _Ty, size_t _Size> _NODISCARD _CONSTEXPR17 reverse_iterator<_Ty*> rend(_Ty (&_Array)[_Size])
{
...
template<class _Elem> _NODISCARD _CONSTEXPR17 reverse_iterator<const _Elem*> rend(::std::initializer_list<_Elem> _Ilist)
{
    return reverse_iterator<const _Elem*>(_Ilist.begin());
}

template<class _Container> _NODISCARD _CONSTEXPR17 auto crbegin(const _Container& _Cont) -> decltype(_RPC rbegin(_Cont))
{
    return _RPC rbegin(_Cont);
}

template<class _Container> _NODISCARD _CONSTEXPR17 auto crend(const _Container& _Cont) -> decltype(_RPC rend(_Cont))
{
    return _RPC rend(_Cont);
}

template<class _Container> _NODISCARD constexpr auto size(const _Container& _Cont) -> decltype(_Cont.size())
{
    return _Cont.size();
}

template<class _Ty, size_t _Size> _NODISCARD constexpr size_t size(const _Ty (&)[_Size]) noexcept
{
    return _Size;
}
...
template<class _Container>
_NODISCARD constexpr auto
ssize(const _Container& _Cont) -> ::std::common_type_t<ptrdiff_t, make_signed_t<decltype(_Cont.size())>>
{
    using _Common = ::std::common_type_t<ptrdiff_t, make_signed_t<decltype(_Cont.size())>>;
    return static_cast<_Common>(_Cont.size());
}

template<class _Ty, ptrdiff_t _Size> _NODISCARD constexpr ptrdiff_t ssize(const _Ty (&)[_Size]) noexcept
{
    return _Size;
}
#endif // _HAS_CXX20

template<class _Container> _NODISCARD constexpr auto empty(const _Container& _Cont) -> decltype(_Cont.empty())
{
    return _Cont.empty();
}

template<class _Ty, size_t _Size> _NODISCARD constexpr bool empty(const _Ty (&)[_Size]) noexcept
{
    return false;
}

template<class _Elem> _NODISCARD constexpr bool empty(::std::initializer_list<_Elem> _Ilist) noexcept
{
    return _Ilist.size() == 0;
}

template<class _Container> _NODISCARD constexpr auto data(_Container& _Cont) -> decltype(_Cont.data())
{
    return _Cont.data();
}

template<class _Container> _NODISCARD constexpr auto data(const _Container& _Cont) -> decltype(_Cont.data())
{
    return _Cont.data();
}

template<class _Ty, size_t _Size> _NODISCARD constexpr _Ty* data(_Ty (&_Array)[_Size]) noexcept
{
    return _Array;
}

template<class _Elem> _NODISCARD constexpr const _Elem* data(::std::initializer_list<_Elem> _Ilist) noexcept
{
    return _Ilist.begin();
}

#ifdef __cpp_lib_concepts
template<class _Ty> _NODISCARD _Ty _Fake_decay_copy(_Ty) noexcept;
...
namespace ranges
{
    template<class> inline constexpr bool _Has_complete_elements = false;
    ...
    namespace _Begin
    {
        template<class _Ty> void begin(_Ty&) = delete;
        template<class _Ty> void begin(const _Ty&) = delete;
        ...
        private:
            ...
            template<class _Ty> _NODISCARD static _CONSTEVAL _Choice_t<_St> _Choose() noexcept
            {
    ...
    template<class _Ty> using iterator_t = decltype(_RANGES begin(declval<_Ty&>()));
    ...
    namespace _Unchecked_begin
    {
        ...
        template<class _Ty>
        concept _Can_begin = requires(_Ty& __t) { _Get_unwrapped(_RANGES begin(__t)); };
    ...
    namespace _End
    {
        template<class _Ty> void end(_Ty&) = delete;
        template<class _Ty> void end(const _Ty&) = delete;
    ...
    namespace _Unchecked_end
    {
        ...
        template<class _Ty>
        concept _Can_end = requires(_Ty& __t) { _Get_unwrapped(_RANGES end(__t)); };
    ...
    template<range _Rng> using sentinel_t = decltype(_RANGES end(declval<_Rng&>()));

    template<range _Rng> using range_difference_t = iter_difference_t<iterator_t<_Rng>>;

    template<range _Rng> using range_value_t = iter_value_t<iterator_t<_Rng>>;

    template<range _Rng> using range_reference_t = iter_reference_t<iterator_t<_Rng>>;

    template<range _Rng> using range_rvalue_reference_t = iter_rvalue_reference_t<iterator_t<_Rng>>;
    ...
    namespace _Rbegin
    {
        template<class _Ty> void rbegin(_Ty&) = delete;
        template<class _Ty> void rbegin(const _Ty&) = delete;
    ...
    namespace _Rend
    {
        template<class _Ty> void rend(_Ty&) = delete;
        template<class _Ty> void rend(const _Ty&) = delete;
        ...
        private:
            ...
            template<class _Ty> _NODISCARD static _CONSTEVAL _Choice_t<_St> _Choose() noexcept
            {
                ...
                {
                    return {_St::_Make_reverse, noexcept(_RPC make_reverse_iterator(_RANGES begin(declval<_Ty>())))};
                }
                else
                {
                    return {_St::_None};
    ...
    namespace _Size
    {
        template<class _Ty> void size(_Ty&) = delete;
        template<class _Ty> void size(const _Ty&) = delete;
        ...
        private:
            ...
            template<class _Ty> _NODISCARD static _CONSTEVAL _Choice_t<_St> _Choose() noexcept
            {
                ...
                {
                    return {_St::_Subtract, noexcept(_RANGES end(declval<_Ty>()) - _RANGES begin(declval<_Ty>()))};
                }
                else
                {
                    return {_St::_None};
    ...
    namespace _Empty
    {
        ...
        private:
            ...
            template<class _Ty> _NODISCARD static _CONSTEVAL _Choice_t<_St> _Choose() noexcept
            {
                ...
                {
                    constexpr auto _Nothrow
                        = noexcept(static_cast<bool>(_RANGES begin(declval<_Ty>()) == _RANGES end(declval<_Ty>())));
    ...
    template<sized_range _Rng> using range_size_t = decltype(_RANGES size(declval<_Rng&>()));
    ...
    concept _Derived_from_specialization_of
        = requires(const _Ty& _Obj) { _Derived_from_specialization_impl<_Template>(_Obj); };
    ...
    public:
        ...
        template<input_or_output_iterator _It> constexpr void operator()(_It& _Where, iter_difference_t<_It> _Off) const
        {
            ...
            else
            {
                ...
                constexpr bool _Need_rewrap
                    = !::std::is_reference_v<decltype(_Get_unwrapped_n(::std::move(_Where), _Off))>;
        ...
        constexpr void operator()(_It& _Where, _Se _Last) const
        {
            ...
            else
            {
                ...
                constexpr bool _Need_rewrap
                    = !::std::is_reference_v<decltype(_Get_unwrapped(static_cast<_It&&>(_Where)))>;
    ...
    private:
        ...
        _NODISCARD static constexpr iter_difference_t<_It>
        _Distance_unchecked(_It _First, const _Se _Last) noexcept(_Nothrow_distance<_It, _Se>)
        {
        ...
        template<class _It, class _Se>
        static constexpr bool _Nothrow_distance
            = noexcept(noexcept(++declval<_Unwrapped_t<_It>&>() != declval<const _Unwrapped_t<_Se>&>()));
        template<class _It, sized_sentinel_for<_It> _Se>
        static constexpr bool _Nothrow_distance<_It, _Se> = noexcept(noexcept(declval<_Se&>() - declval<_It&>()));

        template<class _Rng>
        static constexpr bool _Nothrow_size = _Nothrow_distance<iterator_t<_Rng>, sentinel_t<_Rng>>;
        template<sized_range _Rng> static constexpr bool _Nothrow_size<_Rng> = noexcept(_RANGES size(declval<_Rng&>()));
    };
    ...
    public:
        ...
        template<input_or_output_iterator _It> _NODISCARD constexpr _It operator()(_It _Where) const
        {
            ++_Where;
            return _Where;
        }
    ...
    public:
        ...
        template<bidirectional_iterator _It> _NODISCARD constexpr _It operator()(_It _Where) const
        {
            --_Where;
            return _Where;
        }
    ...
    _NODISCARD constexpr _It _Find_last_iterator(const _It& _First, const _Se& _Last,
                                                 const iter_difference_t<_It> _Count)
    {
...
namespace ranges
{
    ...
    public:
        ...
        constexpr explicit _Subrange_base(const _Size_type& _Size_) noexcept
            : _Size(_Size_)
        {
        }
    };
    ...
    private:
        ...
        constexpr subrange(true_type, _Rng&& _Val)
            : subrange(::std::forward<_Rng>(_Val), _RANGES size(_Val))
        {
        ...
        template<class _Rng>
        constexpr subrange(false_type, _Rng&& _Val)
            : subrange(_RANGES begin(_Val), _RANGES end(_Val))
        {
    ...
    public:
        ...
        _NODISCARD constexpr _Se end() const { return _Last; }

        _NODISCARD constexpr bool empty() const { return _First == _Last; }
    ...
    template<input_or_output_iterator _It, sentinel_for<_It> _Se> subrange(_It, _Se) -> subrange<_It, _Se>;
    ...
    subrange(_Rng&&) -> subrange<iterator_t<_Rng>, sentinel_t<_Rng>,
                                 (sized_range<_Rng> || sized_sentinel_for<sentinel_t<_Rng>, iterator_t<_Rng>>)
                                     ? subrange_kind::sized
...
template<class _It, class _Se, ranges::subrange_kind _Ki> struct tuple_element<0, const ranges::subrange<_It, _Se, _Ki>>
{
    using type = _It;
};

template<class _It, class _Se, ranges::subrange_kind _Ki> struct tuple_element<1, const ranges::subrange<_It, _Se, _Ki>>
{
    using type = _Se;
};
...
namespace ranges
{
    struct dangling
    {
        ...
        template<class... _Args> constexpr dangling(_Args&&...) noexcept { }
    };
...
template<class> struct _Move_iterator_category
{
};
...
template<class _Iter> struct _Move_iterator_category
{
    using iterator_category = _Iter_cat_t<_Iter>;
};
#endif // __cpp_lib_concepts

template<class _Iter> class move_iterator : public _Move_iterator_category<_Iter>
{
public:
...
#else  // ^^^ __cpp_lib_concepts / !__cpp_lib_concepts vvv
    using reference = ::std::conditional_t<::std::is_reference_v<_Iter_ref_t<_Iter>>,
                                           remove_reference_t<_Iter_ref_t<_Iter>>&&, _Iter_ref_t<_Iter>>;
#endif // __cpp_lib_concepts

    _CONSTEXPR17 move_iterator() = default;

    _CONSTEXPR17 explicit move_iterator(_Iter _Right) noexcept(
        ::std::is_nothrow_move_constructible_v<_Iter>) // strengthened
...
#ifdef __cpp_lib_concepts
    _NODISCARD constexpr const iterator_type& base() const& noexcept { return _Current; }
    _NODISCARD constexpr iterator_type
    base() && noexcept(::std::is_nothrow_move_constructible_v<_Iter>) /* strengthened */
    {
        return ::std::move(_Current);
    }
#else  // ^^^ __cpp_lib_concepts / !__cpp_lib_concepts vvv
    _NODISCARD _CONSTEXPR17 iterator_type base() const { return _Current; }
#endif // __cpp_lib_concepts
    ...
    _CXX20_DEPRECATE_MOVE_ITERATOR_ARROW _NODISCARD _CONSTEXPR17 pointer operator->() const { return _Current; }
    ...
    _NODISCARD auto operator==(_Default_sentinel _Sentinel) const noexcept -> decltype(declval<const _Iter2&>()
                                                                                       == _Sentinel)
    {
    ...
    _NODISCARD friend constexpr difference_type operator-(const move_iterator& _Left,
                                                          const move_sentinel<_Sent>& _Right)
    {
        return _Left._Current - _Right._Get_last();
    }
...
_NODISCARD _CONSTEXPR17 auto operator-(const move_iterator<_Iter1>& _Left,
                                       const move_iterator<_Iter2>& _Right) -> decltype(_Left.base() - _Right.base())
{
    return _Left.base() - _Right.base();
}

template<class _Iter>
_NODISCARD _CONSTEXPR17 move_iterator<_Iter> operator+(typename move_iterator<_Iter>::difference_type _Off,
...
template<class _Iter> _NODISCARD _CONSTEXPR17 move_iterator<_Iter> make_move_iterator(_Iter _It)
...
template<class _Iter> inline constexpr bool _Iterator_is_contiguous = contiguous_iterator<_Iter>;

template<class _Iter> _NODISCARD constexpr auto _To_address(const _Iter& _Val) noexcept
{
...
template<class _Iter> _INLINE_VAR constexpr bool _Iterator_is_contiguous = ::std::is_pointer_v<_Iter>;
...
template<class _Iter> _NODISCARD constexpr auto _To_address(const move_iterator<_Iter>& _Val) noexcept
{
...
_INLINE_VAR constexpr bool _Is_pointer_address_convertible
    = ::std::is_void_v<_Source>
      || ::std::is_void_v<_Dest>
...
template<class _Source, class _Dest, class _SourceRef, class _DestRef> struct _Trivial_cat
{
...
template<class _SourceIt, class _DestIt> struct _Iter_move_cat<_SourceIt, _DestIt, false> : _False_trivial_cat
{
};
...
template<class _SourceIt, class _DestIt> struct _Iter_copy_cat<_SourceIt, _DestIt, false> : _False_trivial_cat
{
};
...
template<class _CtgIt, class _OutCtgIt> _OutCtgIt _Copy_memmove(_CtgIt _First, _CtgIt _Last, _OutCtgIt _Dest)
{
...
template<class _It, bool _RequiresMutable = false> _INLINE_VAR constexpr bool _Is_vb_iterator = false;

template<class _InIt, class _OutIt> _CONSTEXPR20 _OutIt _Copy_unchecked(_InIt _First, _InIt _Last, _OutIt _Dest)
{
...
template<class _InIt, class _OutIt> _CONSTEXPR20 _OutIt copy(_InIt _First, _InIt _Last, _OutIt _Dest)
{ // copy [_First, _Last) to [_Dest, ...)
    _Adl_verify_range(_First, _Last);
...
namespace ranges
{
    ...
    template<class _In, class _Out> struct in_out_result
    {
    ...
    template<class _In, class _Out> using copy_result = in_out_result<_In, _Out>;
...
template<class _CtgIt1, class _CtgIt2> _CtgIt2 _Copy_backward_memmove(_CtgIt1 _First, _CtgIt1 _Last, _CtgIt2 _Dest)
{
...
template<class _BidIt1, class _BidIt2> _CONSTEXPR20 _BidIt2 copy_backward(_BidIt1 _First, _BidIt1 _Last, _BidIt2 _Dest)
{
    // copy [_First, _Last) backwards to [..., _Dest)
    _Adl_verify_range(_First, _Last);
    const auto _UFirst = _Get_unwrapped(_First);
...
template<class _InIt, class _OutIt> _CONSTEXPR20 _OutIt _Move_unchecked(_InIt _First, _InIt _Last, _OutIt _Dest)
{
...
template<class _InIt, class _OutIt> _CONSTEXPR20 _OutIt move(_InIt _First, _InIt _Last, _OutIt _Dest)
{
    // move [_First, _Last) to [_Dest, ...)
    _Adl_verify_range(_First, _Last);
    const auto _UFirst = _Get_unwrapped(_First);
...
template<class _BidIt1, class _BidIt2> _CONSTEXPR20 _BidIt2 move_backward(_BidIt1 _First, _BidIt1 _Last, _BidIt2 _Dest)
{
    // move [_First, _Last) backwards to [..., _Dest)
    _Adl_verify_range(_First, _Last);
    const auto _UFirst = _Get_unwrapped(_First);
...
#ifdef __cpp_char8_t
template<> struct _Is_character<char8_t> : true_type
{
}; // UTF-8 code units are sort-of characters
#endif // __cpp_char8_t

template<class _Ty> struct _Is_character_or_bool : _Is_character<_Ty>::type
{
};

template<> struct _Is_character_or_bool<bool> : true_type
{
};

template<class _Ty> struct _Is_character_or_byte_or_bool : _Is_character_or_bool<_Ty>::type
{
};

#ifdef __cpp_lib_byte
template<> struct _Is_character_or_byte_or_bool<::std::byte> : true_type
{
};
#endif // __cpp_lib_byte
...
_INLINE_VAR constexpr bool _Fill_memset_is_safe
    = ::std::conjunction_v<::std::is_scalar<_Ty>,
                           _Is_character_or_byte_or_bool<_Unwrap_enum_t<remove_reference_t<_Iter_ref_t<_FwdIt>>>>,
                           ::std::negation<::std::is_volatile<remove_reference_t<_Iter_ref_t<_FwdIt>>>>,
                           ::std::is_assignable<_Iter_ref_t<_FwdIt>, const _Ty&>>;

template<class _FwdIt, class _Ty> _INLINE_VAR constexpr bool _Fill_memset_is_safe<_FwdIt, _Ty, false> = false;

template<class _FwdIt, class _Ty, bool = _Iterator_is_contiguous<_FwdIt>>
_INLINE_VAR constexpr bool _Fill_zero_memset_is_safe
    = ::std::conjunction_v<::std::is_scalar<_Ty>, ::std::is_scalar<_Iter_value_t<_FwdIt>>,
                           ::std::negation<::std::is_member_pointer<_Iter_value_t<_FwdIt>>>,
                           ::std::negation<::std::is_volatile<remove_reference_t<_Iter_ref_t<_FwdIt>>>>,
                           ::std::is_assignable<_Iter_ref_t<_FwdIt>, const _Ty&>>;

template<class _FwdIt, class _Ty> _INLINE_VAR constexpr bool _Fill_zero_memset_is_safe<_FwdIt, _Ty, false> = false;

template<class _CtgIt, class _Ty> void _Fill_memset(_CtgIt _Dest, const _Ty _Val, const size_t _Count)
{
...
template<class _CtgIt> void _Fill_zero_memset(_CtgIt _Dest, const size_t _Count)
{
    _CSTD memset(_To_address(_Dest), 0, _Count * sizeof(_Iter_value_t<_CtgIt>));
}

template<class _Ty> _NODISCARD bool _Is_all_bits_zero(const _Ty& _Val)
{
...
template<class _FwdIt, class _Ty> _CONSTEXPR20 void fill(const _FwdIt _First, const _FwdIt _Last, const _Ty& _Val)
{
...
template<class _Ty1, class _Ty2, class = void> _INLINE_VAR constexpr bool _Can_compare_with_operator_equal = false;
...
_INLINE_VAR constexpr bool _Can_memcmp_elements = ::std::is_same_v<_Elem1, bool> || ::std::is_same_v<_Elem2, bool>
                                                  || static_cast<_Elem1>(-1) == static_cast<_Elem2>(-1);
#if defined(__clang__) || defined(__GNUC__) || defined(__GNUG__)
#else
#pragma warning(pop)
#endif
...
template<> inline constexpr bool _Can_memcmp_elements<::std::byte, ::std::byte, false> = true;
#endif // __cpp_lib_byte
...
template<class _Elem1, class _Elem2> _INLINE_VAR constexpr bool _Can_memcmp_elements<_Elem1, _Elem2, false> = false;
...
template<class _Elem1, class _Elem2, class _Pr> _INLINE_VAR constexpr bool _Can_memcmp_elements_with_pred = false;

// With ::std::equal_to<_Elem3> we need to make sure that both _Elem1 and _Elem2 are convertible to _Elem3 without
// changing object representation (we use _Iter_copy_cat for this task) and _Elem3 can be safely memcmp'ed with itself
...
_INLINE_VAR constexpr bool _Can_memcmp_elements_with_pred<_Elem1, _Elem2, ::std::equal_to<_Elem3>>
    = _Iter_copy_cat<_Elem1*, _Elem3*>::_Bitcopy_constructible
      ...
      && _Can_memcmp_elements<::std::remove_cv_t<_Elem3>, ::std::remove_cv_t<_Elem3>>;
...
template<class _CtgIt1, class _CtgIt2> _NODISCARD int _Memcmp_ranges(_CtgIt1 _First1, _CtgIt1 _Last1, _CtgIt2 _First2)
{
...
template<class _InIt1, class _InIt2, class _Pr>
_NODISCARD _CONSTEXPR20 bool equal(const _InIt1 _First1, const _InIt1 _Last1, const _InIt2 _First2, const _InIt2 _Last2,
                                   _Pr _Pred)
{
...
namespace ranges
{
    ...
    template<class _In1, class _In2> struct in_in_result
    {
    ...
    template<class _In1, class _In2> using mismatch_result = in_in_result<_In1, _In2>;
    ...
    public:
        ...
        _NODISCARD constexpr mismatch_result<_It1, _It2> operator()(_It1 _First1, _Se1 _Last1,
            ...
            if constexpr(sized_sentinel_for<_Se1, _It1> && sized_sentinel_for<_Se2, _It2>)
            {
                ...
                auto _Result
                    = _RANGES _Mismatch_n(_Get_unwrapped(::std::move(_First1)), _Get_unwrapped(::std::move(_First2)),
                                          _Count1, _Pass_fn(_Pred), _Pass_fn(_Proj1), _Pass_fn(_Proj2));
            ...
            {
                auto _Result
                    = _RANGES _Mismatch_4(_Get_unwrapped(::std::move(_First1)), _Get_unwrapped(::std::move(_Last1)),
        ...
        _NODISCARD constexpr mismatch_result<borrowed_iterator_t<_Rng1>, borrowed_iterator_t<_Rng2>> operator()(
            ...
            else
            {
                ...
                auto _Result = _RANGES _Mismatch_4(_Get_unwrapped(::std::move(_First1)), _Uend(_Range1),
                                                   _Get_unwrapped(::std::move(_First2)), _Uend(_Range2),
                                                   _Pass_fn(_Pred), _Pass_fn(_Proj1), _Pass_fn(_Proj2));
                _Seek_wrapped(_First1, ::std::move(_Result.in1));
                _Seek_wrapped(_First2, ::std::move(_Result.in2));
                return {::std::move(_First1), ::std::move(_First2)};
...
_INLINE_VAR constexpr bool _Lex_compare_memcmp_classify_elements
    = ::std::conjunction_v<_Is_character_or_bool<_Elem1>, _Is_character_or_bool<_Elem2>, ::std::is_unsigned<_Elem1>,
                           ::std::is_unsigned<_Elem2>>;

#ifdef __cpp_lib_byte
template<> inline constexpr bool _Lex_compare_memcmp_classify_elements<::std::byte, ::std::byte> = true;
#endif // __cpp_lib_byte

template<class _Elem1, class _Elem2, class _Pr> struct _Lex_compare_memcmp_classify_pred
{
    using _Pred = void;
};
...
template<class _Elem1, class _Elem2> struct _Lex_compare_memcmp_classify_pred<_Elem1, _Elem2, ::std::less<>>
{
...
template<class _Elem1, class _Elem2> struct _Lex_compare_memcmp_classify_pred<_Elem1, _Elem2, ::std::greater<>>
{
    using _Pred
        = ::std::conditional_t<_Lex_compare_memcmp_classify_elements<_Elem1, _Elem2>, ::std::greater<int>, void>;
};

#ifdef __cpp_lib_concepts
template<class _Elem1, class _Elem2> struct _Lex_compare_memcmp_classify_pred<_Elem1, _Elem2, _RANGES ::std::less>
{
    using _Pred = ::std::conditional_t<_Lex_compare_memcmp_classify_elements<_Elem1, _Elem2>, ::std::less<int>, void>;
};

template<class _Elem1, class _Elem2> struct _Lex_compare_memcmp_classify_pred<_Elem1, _Elem2, _RANGES ::std::greater>
{
    using _Pred
        = ::std::conditional_t<_Lex_compare_memcmp_classify_elements<_Elem1, _Elem2>, ::std::greater<int>, void>;
};
...
using _Lex_compare_memcmp_classify = ::std::conditional_t<
    _Iterators_are_contiguous<_It1, _It2> && !_Iterator_is_volatile<_It1> && !_Iterator_is_volatile<_It2>,
    typename _Lex_compare_memcmp_classify_pred<_Iter_value_t<_It1>, _Iter_value_t<_It2>, _Pr>::_Pred, void>;

template<class _InIt1, class _InIt2, class _Pr>
_NODISCARD _CONSTEXPR20 bool lexicographical_compare(const _InIt1 _First1, const _InIt1 _Last1, const _InIt2 _First2,
                                                     const _InIt2 _Last2, _Pr _Pred)
{
...
#ifdef __cpp_lib_concepts
template<class _Elem1, class _Elem2, class _Cmp> struct _Lex_compare_three_way_memcmp_classify_comp
{
    using _Comp = void;
};
...
{
    using _Comp = ::std::conditional_t<_Lex_compare_memcmp_classify_elements<_Elem1, _Elem2>
...
{
    using _Comp = ::std::conditional_t<_Lex_compare_memcmp_classify_elements<_Elem1, _Elem2>
                                           && _Can_strong_order<_Elem1, _Elem2>,
...
{
    using _Comp = ::std::conditional_t<_Lex_compare_memcmp_classify_elements<_Elem1, _Elem2>
                                           && _Can_partial_order<_Elem1, _Elem2>,
                                       _Partial_order::_Cpo, void>;
};

template<class _It1, class _It2, class _Cmp>
using _Lex_compare_three_way_memcmp_classify = ::std::conditional_t<
    _Iterators_are_contiguous<_It1, _It2> && !_Iterator_is_volatile<_It1> && !_Iterator_is_volatile<_It2>,
    typename _Lex_compare_three_way_memcmp_classify_comp<_Iter_value_t<_It1>, _Iter_value_t<_It2>, _Cmp>::_Comp, void>;
...
_NODISCARD constexpr auto lexicographical_compare_three_way(const _InIt1 _First1, const _InIt1 _Last1,
                                                            ...
                                                            _Cmp _Comp) -> decltype(_Comp(*_First1, *_First2))
{
    _Adl_verify_range(_First1, _Last1);
    _Adl_verify_range(_First2, _Last2);
    auto _UFirst1 = _Get_unwrapped(_First1);
...
template<class _InIt, class _Ty> _NODISCARD constexpr bool _Within_limits(const _InIt&, const _Ty& _Val)
{
...
template<class _InIt, class _Ty> _NODISCARD _CONSTEXPR20 _InIt find(_InIt _First, const _InIt _Last, const _Ty& _Val)
...
#if _HAS_CXX17
template<class _ExPo, class _FwdIt, class _Ty, _Enable_if_execution_policy_t<_ExPo> = 0>
_NODISCARD _Iter_diff_t<_FwdIt> count(_ExPo&& _Exec, const _FwdIt _First, const _FwdIt _Last,
...
enum class _TrimResult : unsigned char
{
    _KeepTrimming,
...
_NODISCARD _CONSTEXPR20 _TrimResult _Trim_equal(_BidIt1& _First1, _BidIt1& _Back1, _BidIt2& _First2, _BidIt2& _Back2,
                                                _Pr _Pred)
{
...
template<class _BidIt1, class _BidIt2, class _Pr>
_NODISCARD _CONSTEXPR20 _TrimResult _Trim_reversed(_BidIt1& _First1, _BidIt1& _Back1, _BidIt2& _First2, _BidIt2& _Back2,
                                                   _Pr _Pred)
{
...
template<class _BidIt1, class _BidIt2, class _Pr>
_NODISCARD _CONSTEXPR20 _TrimResult _Trim_completely(_BidIt1& _First1, _BidIt1& _Back1, _BidIt2& _First2,
                                                     _BidIt2& _Back2, _Pr _Pred)
{
...
_NODISCARD _CONSTEXPR20 bool _Check_match_counts(_FwdIt1 _First1, _FwdIt1 _Last1, _FwdIt2 _First2, _FwdIt2 _Last2,
                                                 _Pr _Pred)
{
...
template<class _BidIt> _CONSTEXPR20 void reverse(const _BidIt _First, const _BidIt _Last)
{ // reverse elements in [_First, _Last)
    ...
    constexpr bool _Allow_vectorization
        = ::std::conjunction_v<::std::bool_constant<_Iterator_is_contiguous<decltype(_UFirst)>>,
...
template<class _FwdIt> _CONSTEXPR20 _FwdIt rotate(_FwdIt _First, _FwdIt _Mid, _FwdIt _Last)
{
...
template<class _InIt, class _Pr> _NODISCARD _CONSTEXPR20 _InIt find_if(_InIt _First, const _InIt _Last, _Pr _Pred)
...
namespace ranges
{
    ...
    public:
        ...
        _NODISCARD constexpr _It operator()(_It _First, _Se _Last, _Pr _Pred, _Pj _Proj = {}) const
        {
            _Adl_verify_range(_First, _Last);
            auto _UResult
                = _RANGES _Find_if_unchecked(_Get_unwrapped(::std::move(_First)), _Get_unwrapped(::std::move(_Last)),
        ...
        template<input_range _Rng, class _Pj = identity, indirect_unary_predicate<projected<iterator_t<_Rng>, _Pj>> _Pr>
        _NODISCARD constexpr borrowed_iterator_t<_Rng> operator()(_Rng&& _Range, _Pr _Pred, _Pj _Proj = {}) const
        {
            auto _First = _RANGES begin(_Range);
            auto _UResult = _RANGES _Find_if_unchecked(_Get_unwrapped(::std::move(_First)), _Uend(_Range),
                                                       _Pass_fn(_Pred), _Pass_fn(_Proj));
    ...
    public:
        ...
        _NODISCARD constexpr _It operator()(_It _First, _Se _Last, _Pr _Pred, _Pj _Proj = {}) const
        {
            ...
            auto _UResult
                = _Find_if_not_unchecked(_Get_unwrapped(::std::move(_First)), _Get_unwrapped(::std::move(_Last)),
                                         _Pass_fn(_Pred), _Pass_fn(_Proj));
        ...
        _NODISCARD constexpr borrowed_iterator_t<_Rng> operator()(_Rng&& _Range, _Pr _Pred, _Pj _Proj = {}) const
        {
            ...
            auto _UResult = _Find_if_not_unchecked(_Get_unwrapped(::std::move(_First)), _Uend(_Range), _Pass_fn(_Pred),
                                                   _Pass_fn(_Proj));
    ...
    public:
        ...
        _NODISCARD constexpr subrange<_It1> operator()(_It1 _First1, _Se1 _Last1, _It2 _First2, _Se2 _Last2,
            ...
            {
                auto _UResult
                    = _Search_unsized(::std::move(_UFirst1), ::std::move(_ULast1), ::std::move(_UFirst2),
        ...
        _NODISCARD constexpr borrowed_subrange_t<_Rng1> operator()(
            ...
            if constexpr(sized_range<_Rng1> && sized_range<_Rng2>)
            {
                ...
                auto _UResult
                    = _Search_sized(_Ubegin(_Range1), _Uend(_Range1), _Count1, _Ubegin(_Range2), _Uend(_Range2),
                                    _Count2, _Pass_fn(_Pred), _Pass_fn(_Proj1), _Pass_fn(_Proj2));
    ...
    private:
        ...
        _NODISCARD static constexpr subrange<_It1>
        _Search_sized(_It1 _First1, const _Se1 _Last1, iter_difference_t<_It1> _Count1, _It2 _First2, const _Se2 _Last2,
                      const iter_difference_t<_It2> _Count2, _Pr _Pred, _Pj1 _Proj1, _Pj2 _Proj2)
        {
        ...
        template<class _It1, class _Se1, class _It2, class _Se2, class _Pr, class _Pj1, class _Pj2>
        _NODISCARD static constexpr subrange<_It1> _Search_unsized(_It1 _First1, const _Se1 _Last1, _It2 _First2,
                                                                   const _Se2 _Last2, _Pr _Pred, _Pj1 _Proj1,
                                                                   _Pj2 _Proj2)
        {
            _STL_INTERNAL_STATIC_ASSERT(forward_iterator<_It1>);
            _STL_INTERNAL_STATIC_ASSERT(sentinel_for<_Se1, _It1>);
...
#endif // _HAS_CXX20
        {
            __std_swap_ranges_trivially_swappable_noalias(_To_address(_First1), _To_address(_Last1),
                                                          _To_address(_First2));
...
struct _Has_allocator_type<_Ty, _Alloc, ::std::void_t<typename _Ty::allocator_type>>
    : ::std::is_convertible<_Alloc, typename _Ty::allocator_type>::type
{
}; // tests for suitable _Ty::allocator_type
...
template<class _Ty, class _Alloc> _INLINE_VAR constexpr bool uses_allocator_v = uses_allocator<_Ty, _Alloc>::value;
