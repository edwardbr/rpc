...
namespace rpc
{
    template<typename Type> struct type_id
    {
        // keep it public
        uint64_t id = 0;

        type_id() = default;
        type_id(uint64_t initial_id)
            : id(initial_id)
        {
        }
    ...
    {
        zone() = default;
        zone(const type_id<ZoneId>& other)
            : type_id<ZoneId>(other)
        {
        }
    ...
    struct destination_zone : public type_id<DestinationZoneId>
    {
        ...
        destination_zone(const type_id<DestinationZoneId>& other)
            : type_id<DestinationZoneId>(other)
        {
        }
    ...
    struct destination_channel_zone : public type_id<DestinationChannelZoneId>
    {
        ...
        destination_channel_zone(const type_id<DestinationChannelZoneId>& other)
            : type_id<DestinationChannelZoneId>(other)
        {
        }
    ...
    {
        caller_zone() = default;
        caller_zone(const type_id<CallerZoneId>& other)
            : type_id<CallerZoneId>(other)
        {
        }

        type_id<CallerChannelZoneId> as_caller_channel() const { return {id}; }
        type_id<DestinationZoneId> as_destination() const
        {
            return {id};
    ...
    struct caller_channel_zone : public type_id<CallerChannelZoneId>
    {
        ...
        caller_channel_zone(const type_id<CallerChannelZoneId>& other)
            : type_id<CallerChannelZoneId>(other)
        {
        }
    ...
    {
        object() = default;
        object(const type_id<ObjectId>& other)
            : type_id<ObjectId>(other)
        {
        }
    };
    ...
    struct interface_ordinal : public type_id<InterfaceId> // cant use the name interface
    {
        ...
        constexpr interface_ordinal(const type_id<InterfaceId>& other)
            : type_id<InterfaceId>(other)
        {
        }
    };
    ...
    {
        method() = default;
        method(const type_id<MethodId>& other)
            : type_id<MethodId>(other)
        {
        }
    };
...
namespace std
{
    template<typename Type> inline std::string to_string(rpc::type_id<Type> _Val)
    {
        return std::to_string(_Val.get_val());
    }
}

template<> struct std::hash<rpc::zone>
{
    auto operator()(const rpc::zone& item) const noexcept { return (std::size_t)item.id; }
};

template<> struct std::hash<rpc::destination_zone>
{
    auto operator()(const rpc::destination_zone& item) const noexcept { return (std::size_t)item.id; }
};

template<> struct std::hash<rpc::destination_channel_zone>
{
    auto operator()(const rpc::destination_channel_zone& item) const noexcept { return (std::size_t)item.id; }
};

template<> struct std::hash<rpc::caller_zone>
{
    auto operator()(const rpc::caller_zone& item) const noexcept { return (std::size_t)item.id; }
};
...
{
    auto operator()(const rpc::interface_ordinal& item) const noexcept { return (std::size_t)item.id; }
};

template<> struct std::hash<rpc::object>
{
    auto operator()(const rpc::object& item) const noexcept { return (std::size_t)item.id; }
};

template<> struct std::hash<rpc::method>
{
    auto operator()(const rpc::method& item) const noexcept { return (std::size_t)item.id; }
};
