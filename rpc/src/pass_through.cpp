...
pass_through::pass_through(std::shared_ptr<transport> forward,
    ...
    , service_(service)
{
    // Keep self alive initially - will be released when counts reach zero or on errors
...
CORO_TASK(int) pass_through::send(uint64_t protocol_version,
    encoding encoding,
    uint64_t tag,
    ...
    std::vector<rpc::back_channel_entry>& out_back_channel)
{
    ...
    auto result = CO_AWAIT target_transport->send(
...
CORO_TASK(void) pass_through::post(uint64_t protocol_version,
    encoding encoding,
    uint64_t tag,
    caller_channel_zone caller_channel_zone_id,
    caller_zone caller_zone_id,
...
CORO_TASK(int) pass_through::try_cast(
...
CORO_TASK(int) pass_through::add_ref(uint64_t protocol_version,
...
CORO_TASK(int) pass_through::release(uint64_t protocol_version,
    destination_zone destination_zone_id,
    object object_id,
    caller_zone caller_zone_id,
