...
namespace rpc
{
    ...
    object service::get_object_id(shared_ptr<casting_interface> ptr) const 
    {
        ...
        {
            return ptr->get_interface_proxy()->get_object_proxy()->get_object_id();
        }
        return {};
    }
    ...
    void service::clean_up_on_failed_connection(const std::shared_ptr<service_proxy>& destination_zone, rpc::shared_ptr<rpc::casting_interface> input_interface)
    {
    ...
    interface_descriptor service::prepare_remote_input_interface(uint64_t protocol_version, caller_channel_zone caller_channel_zone_id, caller_zone caller_zone_id, const rpc::interface_proxy* base, std::shared_ptr<service_proxy>& destination_zone)
    {
    ...
    interface_descriptor service::prepare_out_param(uint64_t protocol_version, caller_channel_zone caller_channel_zone_id, caller_zone caller_zone_id, const rpc::interface_proxy* base)
    {
        auto object_proxy = base->get_object_proxy();
        auto object_service_proxy = object_proxy->get_service_proxy();
        RPC_ASSERT(object_service_proxy->zone_id_ == zone_id_);
                                        ...
                                        rpc::shared_ptr<object_stub>& stub)
    {
        if(outcall)
        {
            const interface_proxy* interface_proxy = nullptr;
            if(caller_channel_zone_id.is_set() || caller_zone_id.is_set())
            {
                interface_proxy = iface->get_interface_proxy();
            }
            if(interface_proxy)
            {
                return prepare_out_param(protocol_version, caller_channel_zone_id, caller_zone_id, interface_proxy);
            }
        }
