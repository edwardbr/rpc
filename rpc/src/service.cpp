...
namespace rpc
{
    ...
    int service::send(uint64_t protocol_version, encoding encoding, uint64_t tag,
                      ...
                      method method_id, size_t in_size_, const char* in_buf_, std::vector<char>& out_buf_)
    {
        ...
        if(destination_zone_id != zone_id_.as_destination())
        {
            ...
            return other_zone->send(protocol_version, encoding, tag, zone_id_.as_caller_channel(), caller_zone_id,
                                    destination_zone_id, object_id, interface_id, method_id, in_size_, in_buf_,
                                    out_buf_);
        }
        else
        {
            ...
            std::for_each(service_loggers.begin(), service_loggers.end(),
                          [&](const std::shared_ptr<service_logger>& logger) {
                              logger->before_send(caller_zone_id, object_id, interface_id, method_id, in_size_,
                                                  in_buf_ ? in_buf_ : "");
                          });

            auto ret = stub->call(protocol_version, encoding, caller_channel_zone_id, caller_zone_id, interface_id,
                                  method_id, in_size_, in_buf_, out_buf_);
    ...
    void service::clean_up_on_failed_connection(const rpc::shared_ptr<service_proxy>& destination_zone,
    ...
    interface_descriptor service::prepare_remote_input_interface(caller_channel_zone caller_channel_zone_id,
                                                                 caller_zone caller_zone_id, rpc::proxy_base* base,
                                                                 rpc::shared_ptr<service_proxy>& destination_zone)
    {
        ...
        destination_zone = object_service_proxy;
        {
            std::lock_guard g(zone_control);
            auto found
                = other_zones.find({destination_zone_id, caller_zone_id}); // we dont need to get caller id for this
            if(found != other_zones.end())
            {
        ...
        if(auto telemetry_service = rpc::telemetry_service_manager::get(); telemetry_service)
        {
            telemetry_service->on_service_proxy_add_ref(zone_id_, destination_zone_id, destination_channel_zone_id,
        ...
        destination_zone->add_ref(rpc::get_version(), destination_channel_zone_id, destination_zone_id, object_id,
                                  zone_id_.as_caller_channel(), caller_zone_id,
                                                    ...
                                                    caller_zone caller_zone_id, rpc::proxy_base* base)
    {
        ...
        if(object_channel == destination_channel)
        {
            ...
            if(auto telemetry_service = rpc::telemetry_service_manager::get(); telemetry_service)
            {
                telemetry_service->on_service_proxy_add_ref(
                    zone_id_, destination_zone_id, {0}, caller_zone_id, object_id,
                    rpc::add_ref_options::build_caller_route | rpc::add_ref_options::build_destination_route);
            }
#endif
            object_service_proxy->add_ref(protocol_version, {0}, destination_zone_id, object_id, {0}, caller_zone_id,
        ...
        else
        {
                ...
                // and the caller with destination info
                {
                    ...
                    if(found == other_zones.end())
                    {
                        ...
                        if(alternative == other_zones.end()
                           || alternative->first.dest != caller_zone_id.as_destination())
                        {
                        ...
                        RPC_ASSERT(alternative_caller_service_proxy != nullptr
                                   || !!"alternative caller proxy not found");

                        // now make a copy of the original as we need it back
                        caller = alternative_caller_service_proxy->clone_for_zone({caller_channel_zone_id.get_val()},
                                                                                  zone_id_.as_caller());
            ...
            if(auto telemetry_service = rpc::telemetry_service_manager::get(); telemetry_service)
            {
                telemetry_service->on_service_proxy_add_ref(zone_id_, destination_zone_id, {0}, caller_zone_id,
                                                            object_id, rpc::add_ref_options::build_destination_route);
            }
#endif

            // the fork is here so we need to add ref the destination normally with caller info
            // note the caller_channel_zone_id is is this zones id as the caller came from a route via this node
            destination_zone->add_ref(protocol_version, {0}, destination_zone_id, object_id,
                                      zone_id_.as_caller_channel(), caller_zone_id,
                                      rpc::add_ref_options::build_destination_route);
            ...
            if(auto telemetry_service = rpc::telemetry_service_manager::get(); telemetry_service)
            {
                telemetry_service->on_service_proxy_add_ref(zone_id_, destination_zone_id,
                                                            zone_id_.as_destination_channel(), caller_zone_id,
                                                            object_id, rpc::add_ref_options::build_caller_route);
            }
#endif

            // note the caller_channel_zone_id is 0 as the caller came from this route
            caller->add_ref(protocol_version, zone_id_.as_destination_channel(), destination_zone_id, object_id, {0},
                            caller_zone_id, rpc::add_ref_options::build_caller_route);
        }
    ...
    interface_descriptor service::get_proxy_stub_descriptor(
        uint64_t protocol_version, caller_channel_zone caller_channel_zone_id, caller_zone caller_zone_id,
        rpc::casting_interface* iface,
        std::function<rpc::shared_ptr<i_interface_stub>(rpc::shared_ptr<object_stub>)> fn, bool outcall,
        rpc::shared_ptr<object_stub>& stub)
    {
        ...
        auto* pointer = iface->get_address();
        {
            ...
            if(outcall)
            {
                ...
                uint64_t object_channel
                    = caller_channel_zone_id.is_set() ? caller_channel_zone_id.id : caller_zone_id.id;
                RPC_ASSERT(object_channel);
                // and the caller with destination info
                auto found = other_zones.find(
                    {{object_channel}, zone_id_.as_caller()}); // we dont need to get caller id for this
        ...
        if(outcall)
        {
            ...
            if(auto telemetry_service = rpc::telemetry_service_manager::get(); telemetry_service)
            {
                telemetry_service->on_service_proxy_add_ref(zone_id_, zone_id_.as_destination(), {0}, caller_zone_id,
                                                            stub->get_id(), rpc::add_ref_options::build_caller_route);
            }
#endif
            // note the caller_channel_zone_id is 0 as the caller came from this route
            caller->add_ref(protocol_version, {0}, zone_id_.as_destination(), stub->get_id(), {0}, caller_zone_id,
                            rpc::add_ref_options::build_caller_route);
        }
    ...
    int service::try_cast(uint64_t protocol_version, destination_zone destination_zone_id, object object_id,
    ...
    uint64_t service::add_ref(uint64_t protocol_version, destination_channel_zone destination_channel_zone_id,
                              ...
                              add_ref_options build_out_param_channel)
    {
        ...
        if(auto telemetry_service = rpc::telemetry_service_manager::get(); telemetry_service)
        {
            telemetry_service->on_service_add_ref(zone_id_, destination_channel_zone_id, destination_zone_id, object_id,
                                                  caller_channel_zone_id, caller_zone_id, build_out_param_channel);
        }
#endif
        ...
        if(destination_zone_id != zone_id_.as_destination())
        {
            ...
            if(dest_channel == caller_channel && build_channel)
            {
                ...
                {
                    telemetry_service->on_service_proxy_add_ref(zone_id_, destination_zone_id, {0}, caller_zone_id,
                                                                object_id, build_out_param_channel);
                }
#endif
                return destination->add_ref(protocol_version, {0}, destination_zone_id, object_id, {0}, caller_zone_id,
                    ...
                    do
                    {
                                  ...
                                  == (add_ref_options::build_caller_route | add_ref_options::build_destination_route))
                        {
                            ...
                            if(dc == cc)
                            {
                                ...
                                if(auto telemetry_service = rpc::telemetry_service_manager::get(); telemetry_service)
                                {
                                    telemetry_service->on_service_proxy_add_ref(zone_id_, destination_zone_id, {0},
                                                                                caller_zone_id, object_id,
                                                                                build_out_param_channel);
                                }
#endif

                                auto ret = destination->add_ref(protocol_version, {0}, destination_zone_id, object_id,
                                                                {0}, caller_zone_id, build_out_param_channel);
...
#endif
                            destination->add_ref(protocol_version, {0}, destination_zone_id, object_id,
                                                 zone_id_.as_caller_channel(), caller_zone_id,
                                                 add_ref_options::build_destination_route);
                        }
                        ...
                        if(!!(build_out_param_channel & add_ref_options::build_caller_route))
                        {
                            ...
                            {
                                telemetry_service->on_service_proxy_add_ref(
                                    caller->zone_id_, destination_zone_id, zone_id_.as_destination_channel(),
                                    caller_zone_id, object_id, add_ref_options::build_caller_route);
                            }
#endif

                            caller->add_ref(protocol_version, zone_id_.as_destination_channel(), destination_zone_id,
            ...
            else
            {
                ...
                return other_zone->add_ref(protocol_version, {0}, destination_zone_id, object_id,
                                           caller_channel_zone_id, caller_zone_id, build_out_param_channel);
            }
        }
        else
        {
               ...
               && !!(build_out_param_channel & add_ref_options::build_caller_route))
            {
                ...
                {
                    telemetry_service->on_service_proxy_add_ref(zone_id_, destination_zone_id, {0}, caller_zone_id,
                                                                object_id, add_ref_options::build_caller_route);
                }
#endif
                caller->add_ref(protocol_version, {0}, destination_zone_id, object_id, {}, caller_zone_id,
                                add_ref_options::build_caller_route);
            }
    ...
    uint64_t service::release(uint64_t protocol_version, destination_zone destination_zone_id, object object_id,
...
#ifdef USE_RPC_TELEMETRY
            if(auto telemetry_service = rpc::telemetry_service_manager::get(); telemetry_service)
                telemetry_service->on_service_release(zone_id_, other_zone->get_destination_channel_zone_id(),
    ...
    rpc::shared_ptr<service_proxy> service::get_zone_proxy(caller_channel_zone caller_channel_zone_id,
                                                           ...
                                                           caller_zone new_caller_zone_id, bool& new_proxy_added)
    {
    ...
    void service::add_interface_stub_factory(
        std::function<interface_ordinal(uint8_t)> id_getter,
        std::shared_ptr<
            std::function<rpc::shared_ptr<rpc::i_interface_stub>(const rpc::shared_ptr<rpc::i_interface_stub>&)>>
            factory)
    {
#ifdef RPC_V2
