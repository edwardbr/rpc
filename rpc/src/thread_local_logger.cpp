...
thread_local_circular_buffer::thread_local_circular_buffer(size_t size)
    : buffer_(size), thread_id_(std::this_thread::get_id())
{}

void thread_local_circular_buffer::add_entry(int level, const std::string& message, const char* file, int line, const char* function) {
...
void thread_local_circular_buffer::dump_to_file(const std::string& filename) const {
    ...
    for (size_t i = 0; i < entries; ++i) {
        ...
        if (!entry.message.empty()) { // Only dump valid entries
...
void thread_local_logger_manager::dump_all_buffers_with_stacktrace(const std::string& assert_message, const char* file, int line) {
    dump_all_buffers_with_enhanced_stacktrace(assert_message, file, line, "");
}

void thread_local_logger_manager::dump_all_buffers_with_enhanced_stacktrace(const std::string& assert_message, const char* file, int line, const std::string& stack_trace) {
    freeze_all_buffers();
    ...
    // Dump each thread buffer to separate file
    {
        ...
        for (auto& [tid, buffer] : buffers_) {
            ...
            std::string buffer_file = dump_dir + "/thread_" + std::to_string(thread_counter) 
                                    + "_" + thread_id_str + "_" + timestamp + ".log";
