...
namespace rpc
{
    ...
    class host_service_proxy : public service_proxy
    {
        ...
        std::shared_ptr<service_proxy> clone() override {return std::shared_ptr<host_service_proxy>(new host_service_proxy(*this));}

        static std::shared_ptr<service_proxy> create(const char* name, destination_zone host_zone_id, const rpc::shared_ptr<rpc::child_service>& svc);
