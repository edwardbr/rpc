...
namespace rpc::spsc
{
    ...
    public:
        using connection_handler = std::function<CORO_TASK(int)(
            const rpc::interface_descriptor& input_descr, rpc::interface_descriptor& output_interface,
            rpc::shared_ptr<rpc::service> child_service_ptr, std::shared_ptr<channel_manager>)>;
    ...
    private:
        ...
        channel_manager(std::chrono::milliseconds timeout, rpc::shared_ptr<rpc::service> service,
                        queue_type* send_spsc_queue, queue_type* receive_spsc_queue, connection_handler handler);
