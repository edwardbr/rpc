...
public:
    ...
    template<typename U> T* allocate(const size_t n, const U*) const { return allocate(n); }
...
template<class T> struct adl_proof_allocator_provider
{
    struct alloc;
};

// adl_proof_allocator<T> is equality-comparable even if T is ADL-incompatible.
template<class T> using adl_proof_allocator = typename adl_proof_allocator_provider<T>::alloc;

template<class, class = void> constexpr bool is_adl_proof_allocator = false;

template<class Alloc>
constexpr bool is_adl_proof_allocator<Alloc, void_t<typename adl_proof_allocator_provider<typename Alloc::value_type>::alloc>>
    = is_same_v<typename adl_proof_allocator_provider<typename Alloc::value_type>::alloc, Alloc>;

template<class T> struct adl_proof_allocator_provider<T>::alloc
{
    using value_type = T;
    ...
    T* allocate(const size_t n) { return allocator<T>{}.allocate(n); }

#if _HAS_CXX23
    allocation_result<T*> allocate_at_least(const size_t n) { return allocator<T>{}.allocate_at_least(n); }
#endif // _HAS_CXX23

    void deallocate(T* const p, const size_t n) { return allocator<T>{}.deallocate(p, n); }
...
template<class T> struct tagged_nontrivial
{
    tagged_nontrivial() noexcept { }
    tagged_nontrivial(const tagged_nontrivial&) { }
    tagged_nontrivial(tagged_nontrivial&&) noexcept { }
    tagged_nontrivial& operator=(const tagged_nontrivial&) { return *this; }
    tagged_nontrivial& operator=(tagged_nontrivial&&) noexcept { return *this; }
    ~tagged_nontrivial() noexcept { }
};

template<class T> void test_adl_proof_shared_ptr_creation_one()
{ // COMPILE-ONLY
