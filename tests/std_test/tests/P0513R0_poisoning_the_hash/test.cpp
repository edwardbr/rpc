...
template<typename T, typename = void> struct hash_callable : false_type
{
};

template<typename T> struct hash_callable<T, void_t<decltype(declval<hash<T>>()(declval<const T&>()))>> : true_type
{
};

template<typename T, bool NoExcept = true> constexpr bool standard_hash_enabled()
{
