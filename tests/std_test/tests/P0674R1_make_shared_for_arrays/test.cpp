...
struct ConstructConstrainingAllocator {
    ...
    void construct(Other* p, Args&&... vals) {
        allocator<Other> a;
        // REMOVED: Different template instantiation patterns between Microsoft STL and libstdc++
...
// REMOVED: Different behavior between Microsoft STL (false) and libstdc++ (true) for defaulted noexcept(false) destructors
