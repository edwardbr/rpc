...
template<typename T> [[nodiscard]] bool weak_ptr_equal(const weak_ptr<T>& left, const weak_ptr<T>& right)
{
...
template<class AtomicType> void ensure_nonmember_calls_compile()
{
    AtomicType instance;
    ...
    if (atomic_compare_exchange_weak_explicit(&instance, &loaded, loaded, memory_order::relaxed, memory_order::relaxed))
    {
        // intentionally empty
    }
    ...
    if (atomic_compare_exchange_strong_explicit(&instance, &loaded, loaded, memory_order::relaxed, memory_order::relaxed))
    {
        // intentionally empty
    }
}

template<class AtomicType> void ensure_member_calls_compile()
{
    AtomicType instance;
    const AtomicType constInstance;
...
template<class T> void test_adl_proof_atomic_shared_weak_ptr_one()
{ // COMPILE-ONLY
    {
...
void test_adl_proof_atomic_shared_weak_ptr()
{ // COMPILE-ONLY
