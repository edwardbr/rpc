...
TYPED_TEST(remote_type_test, check_for_multiple_sets)
{
    ...
    auto zone_id = i_foo_ptr->get_interface_proxy()->get_object_proxy()->get_service_proxy()->get_zone_id();
...
TYPED_TEST(remote_type_test, check_for_set_multiple_inheritance)
{
    if(!this->get_lib().get_use_host_in_child())
        return;
    auto proxy = this->get_lib().get_example()->get_interface_proxy();        
...
TYPED_TEST(remote_type_test, check_identity)
{
    ...
    auto proxy = lib.get_example()->get_interface_proxy();
