...
public:
    ...
    error_code create_enclave(rpc::shared_ptr<yyy::i_example>& target) override
    {
        ...
        auto err_code = serv->connect_to_zone<rpc::enclave_service_proxy>("an enclave", {++(*zone_gen)}, host, target,
                                                                          enclave_path);
...
public:
    ...
    virtual void SetUp()
    {
        ...
        i_example_ptr_
            = rpc::shared_ptr<yyy::i_example>(new example(nullptr, use_host_in_child_ ? i_host_ptr_ : nullptr));
    }
...
public:
    ...
    void before_send(rpc::caller_zone caller_zone_id, rpc::object object_id, rpc::interface_ordinal interface_id,
                     rpc::method method_id, size_t in_size_, const char* in_buf_)
    {
        logr->info("caller_zone_id {} object_id {} interface_ordinal {} method {} data {}", caller_zone_id.id,
                   object_id.id, interface_id.id, method_id.id, std::string_view(in_buf_, in_size_));
    }

    void after_send(rpc::caller_zone caller_zone_id, rpc::object object_id, rpc::interface_ordinal interface_id,
                    rpc::method method_id, int ret, const std::vector<char>& out_buf_)
    {
        logr->info("caller_zone_id {} object_id {} interface_ordinal {} method {} ret {} data {}", caller_zone_id.id,
                   object_id.id, interface_id.id, method_id.id, ret,
...
public:
    ...
    rpc::shared_ptr<yyy::i_example> create_new_zone()
    {
        ...
        if(CreateNewZoneThenCreateSubordinatedZone)
        {
            ...
            if(example_relay_ptr->create_example_in_subordinate_zone(new_ptr, use_host_in_child_ ? hst : nullptr,
                                                                     ++zone_gen_)
               == rpc::error::OK())
            {
...
TYPED_TEST(remote_type_test, multithreaded_check_sub_subordinate)
{
    ...
    for(auto& thread_target : threads)
    {
        thread_target = std::thread(
            ...
            {
                rpc::shared_ptr<yyy::i_example> new_zone;
                ASSERT_EQ(lib.get_example()->create_example_in_subordinate_zone(new_zone, lib.get_local_host_ptr(),
                                                                                ++(*zone_gen)),
                          rpc::error::OK()); // second level
...
TYPED_TEST(type_test_with_host, call_host_create_enclave_and_throw_away)
{
    ...
    ASSERT_EQ(this->get_lib().get_example()->call_host_create_enclave_and_throw_away(run_standard_tests),
              rpc::error::OK());
}
...
TYPED_TEST(type_test_with_host, look_up_app_and_return_with_nothing)
{
    ...
    ASSERT_EQ(this->get_lib().get_example()->call_host_look_up_app("target", target, run_standard_tests),
              rpc::error::OK());
...
TYPED_TEST(type_test_with_host, create_store_fetch_delete)
{
    ...
    ASSERT_EQ(this->get_lib().get_example()->call_host_set_app("target", target, run_standard_tests), rpc::error::OK());
    ASSERT_EQ(this->get_lib().get_example()->call_host_look_up_app("target", target2, run_standard_tests),
              rpc::error::OK());
    ASSERT_EQ(this->get_lib().get_example()->call_host_unload_app("target"), rpc::error::OK());
