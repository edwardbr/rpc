...
extern "C"
{
    ...
    void on_service_try_cast_host(const char* name, uint64_t zone_id, uint64_t destination_zone_id,
                                  uint64_t caller_zone_id, uint64_t object_id, uint64_t interface_id)
    {
        if(telemetry_service)
            telemetry_service->on_service_try_cast(name, {zone_id}, {destination_zone_id}, {caller_zone_id},
                                                   {object_id}, {interface_id});
    }
    void on_service_add_ref_host(const char* name, uint64_t zone_id, uint64_t destination_channel_zone_id,
                                 uint64_t destination_zone_id, uint64_t object_id, uint64_t caller_channel_zone_id,
                                 uint64_t caller_zone_id, uint64_t options)
    {
        if(telemetry_service)
            telemetry_service->on_service_add_ref(name, {zone_id}, {destination_channel_zone_id}, {destination_zone_id},
                                                  {object_id}, {caller_channel_zone_id}, {caller_zone_id},
                                                  (rpc::add_ref_options)options);
    }
    void on_service_release_host(const char* name, uint64_t zone_id, uint64_t destination_channel_zone_id,
                                 uint64_t destination_zone_id, uint64_t object_id, uint64_t caller_zone_id)
    {
        if(telemetry_service)
            telemetry_service->on_service_release(name, {zone_id}, {destination_channel_zone_id}, {destination_zone_id},
                                                  {object_id}, {caller_zone_id});
    }

    void on_service_proxy_creation_host(const char* name, uint64_t zone_id, uint64_t destination_zone_id,
                                        uint64_t caller_zone_id)
    {
        if(telemetry_service)
            telemetry_service->on_service_proxy_creation(name, {zone_id}, {destination_zone_id}, {caller_zone_id});
    }
    void on_service_proxy_deletion_host(const char* name, uint64_t zone_id, uint64_t destination_zone_id,
                                        uint64_t caller_zone_id)
    {
        if(telemetry_service)
            telemetry_service->on_service_proxy_deletion(name, {zone_id}, {destination_zone_id}, {caller_zone_id});
    }
    void on_service_proxy_try_cast_host(const char* name, uint64_t zone_id, uint64_t destination_zone_id,
                                        uint64_t caller_zone_id, uint64_t object_id, uint64_t interface_id)
    {
        if(telemetry_service)
            telemetry_service->on_service_proxy_try_cast(name, {zone_id}, {destination_zone_id}, {caller_zone_id},
                                                         {object_id}, {interface_id});
    }
    void on_service_proxy_add_ref_host(const char* name, uint64_t zone_id, uint64_t destination_zone_id,
                                       uint64_t destination_channel_zone_id, uint64_t caller_zone_id,
                                       uint64_t object_id, uint64_t options)
    {
        if(telemetry_service)
            telemetry_service->on_service_proxy_add_ref(name, {zone_id}, {destination_zone_id},
                                                        {destination_channel_zone_id}, {caller_zone_id}, {object_id},
                                                        (rpc::add_ref_options)options);
    }
    ...
    {
        if(telemetry_service)
            telemetry_service->on_service_proxy_release(name, {zone_id}, {destination_zone_id},
                                                        {destination_channel_zone_id}, {caller_zone_id}, {object_id});
    }
    ...
    void on_object_proxy_creation_host(uint64_t zone_id, uint64_t destination_zone_id, uint64_t object_id,
                                       int add_ref_done)
    {
        if(telemetry_service)
    ...
    void on_interface_proxy_creation_host(const char* name, uint64_t zone_id, uint64_t destination_zone_id,
                                          uint64_t object_id, uint64_t interface_id)
    {
        if(telemetry_service)
            telemetry_service->on_interface_proxy_creation(name, {zone_id}, {destination_zone_id}, {object_id},
                                                           {interface_id});
    }
    void on_interface_proxy_deletion_host(const char* name, uint64_t zone_id, uint64_t destination_zone_id,
                                          uint64_t object_id, uint64_t interface_id)
    {
        if(telemetry_service)
            telemetry_service->on_interface_proxy_deletion(name, {zone_id}, {destination_zone_id}, {object_id},
                                                           {interface_id});
    }
    void on_interface_proxy_send_host(const char* method_name, uint64_t zone_id, uint64_t destination_zone_id,
                                      uint64_t object_id, uint64_t interface_id, uint64_t method_id)
    {
        if(telemetry_service)
            telemetry_service->on_interface_proxy_send(method_name, {zone_id}, {destination_zone_id}, {object_id},
                                                       {interface_id}, {method_id});
    }

    void on_service_proxy_add_external_ref_host(const char* name, uint64_t zone_id,
                                                uint64_t destination_channel_zone_id, uint64_t destination_zone_id,
                                                uint64_t caller_zone_id, int ref_count)
    {
        if(telemetry_service)
            telemetry_service->on_service_proxy_add_external_ref(name, {zone_id}, {destination_channel_zone_id},
                                                                 {destination_zone_id}, {caller_zone_id}, ref_count);
    }

    void on_service_proxy_release_external_ref_host(const char* name, uint64_t zone_id,
                                                    uint64_t destination_channel_zone_id, uint64_t destination_zone_id,
                                                    uint64_t caller_zone_id, int ref_count)
    {
        if(telemetry_service)
            telemetry_service->on_service_proxy_release_external_ref(
                name, {zone_id}, {destination_channel_zone_id}, {destination_zone_id}, {caller_zone_id}, ref_count);
    }
